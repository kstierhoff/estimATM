...
if (save.figs) {
  ...
  ggsave(nasc.cluster.plot.os,
         filename = here("Figs/fig_nasc_cluster_map_offshore.png"),
...
if (save.figs) {
  ...
  # Save map objects
  save(biomass.dens.figs.os, file = here("Output/biomass_dens_offshore_map_all.Rdata"))  
} else {
  load(here("Output/biomass_dens_map_all.Rdata"))
}
...
save(point.estimates.offshore, 
     file = here("Output/biomass_point_estimates_offshore.Rdata"))

# Save strata nasc summaries to CSV
write_csv(nasc.summ.strata.os, here("Output/nasc_strata_summary_offshore.csv"))
...
# Save results
save(be.stratum.os, 
     file = here("Output/biomass_bootstrap_estimates_stratum_offshore.Rdata"))
...
be.os[is.nan.data.frame(be.os)] <- NA
