...
# Load Lasker NASC data and Saildrone nav
...
sd.intersection <- sd.sf %>% 
  st_intersection(filter(tx.buffer, Type == "Offshore"))
...
sd.summary <- sd.intersection %>% 
  filter(saildrone %in% c(1045, 1046)) %>% 
  group_by(saildrone, label) %>% 
  summarise(
    n = n(),
    min.date = min(datetime),
...
sd.int.final <- filter(sd.intersection, saildrone %in% c(1045, 1046))
