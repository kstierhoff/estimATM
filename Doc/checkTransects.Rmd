...
pacman::p_load(tidyverse,sf,mapview,lubridate,here,knitr,DT,bookdown,
               rnaturalearth,swfscMisc,fs,photobiology,ggspatial,
               leaflet,leaflet.extras,leafem,htmltools,marmap,mapproj,gt,
               beepr,shadowtext,stplanr,tmaptools)
...
# 
# poly.tmp <- st_read(here("Data/GIS/bathy_5m_wc_poly.shp")) %>% 
#   st_cast("POLYGON")
# 
# poly.buff.25 <- st_buffer(poly.tmp, 25/60)
# poly.buff.55 <- st_buffer(poly.buff.25, 30/60)
# poly.buff.100 <- st_buffer(poly.buff.55, 30/60)
# 
# mapview(poly.buff.100, col.regions = "yellow") + 
#   mapview(poly.buff.55, col.regions = "orange") + 
#   mapview(poly.buff.25, col.regions = "green") + 
#   mapview(poly.tmp, col.regions = "gray50") + 
#   mapview(transects, color = "white")
# 
# temp.sp <- as_Spatial(poly.buff.25)
# 
# rgdal::writeOGR(as_Spatial(poly.buff.25), here("Data/GIS/test_buffer.gpx"), layer = "Contour", driver = "GPX")
# 
# poly.25.out <- poly.buff.25 %>% 
#   mutate(
#     long = as.data.frame(st_coordinates(.))$X,
#     lat = as.data.frame(st_coordinates(.))$Y)
# 
# rgdal::writeOGR(temp.sp, "test_buffer", layer = "test", driver = "GPX")
# writeOGR(cities, here("Data/GIS"), "cities", driver="GPX")
```
...
# Read Scott Island MPA shapefile
si.mpa <- st_read(here("Data/GIS/scott_islands_mpa_polygon.shp")) %>% 
  st_difference(filter(countries, name == "Canada"))
...
### Lisa Maria (WA, OR, and Norhern CA)  
