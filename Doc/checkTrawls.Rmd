...
for (i in unique(L.max$scientificName)) {
  ...
  TL <- seq(0, L.max$max.TL[L.max$scientificName == i])
  
  # Calculate weights from lengths
  W <- estimate_weight(i, TL, season = tolower(survey.season))
  
  # Combine results
  lw.df <- bind_rows(lw.df, data.frame(scientificName = i, W, TL))
}

# Convert lengths for plotting
lw.df <- lw.df %>% 
  mutate(
    FL = convert_length(scientificName, TL, "TL", "FL"),
    SL = convert_length(scientificName, TL, "TL", "SL")
  )
...
# lw.df.sub <- filter(lw.df, scientificName == scientificName.sub) %>% 
#   pivot_longer(cols = TL:SL,
#                names_to = "length.type",
#                values_to = "length") %>% 
#   arrange(length.type, W, length)
...
if (nrow(lengths.sub) > 0) {
  ...
  lw.plot <- ggplot() + 
    ...
    geom_line(data = lw.df.sub, aes(SL, W), alpha = 0.75, colour = "gray20", linetype = 'dashed') +
    geom_line(data = lw.df.sub, aes(TL, W), alpha = 0.75, colour = "gray90", linetype = 'dashed') +
    geom_line(data = lw.df.sub, aes(FL, W), alpha = 0.75, colour = "gray90", linetype = 'dashed') +
    # geom_line(data = lw.df.sub, aes(length, W, group = length.type),
...
if (nrow(lengths.sub) > 0) {
  ...
  lw.plot <- ggplot() + 
    ...
    scale_colour_manual(name = "Flagged", values = c("NA" = NA, "TRUE" = "black")) +
    # Plot seasonal length models for each species
    geom_line(data = lw.df.sub, aes(SL, W), alpha = 0.75, colour = "gray90", linetype = 'dashed') +
    geom_line(data = lw.df.sub, aes(TL, W), alpha = 0.75, colour = "gray90", linetype = 'dashed') +
    geom_line(data = lw.df.sub, aes(FL, W), alpha = 0.75, colour = "gray20", linetype = 'dashed') +
    # geom_line(data = lw.df.sub, aes(length, W, group = length.type),
    #           linetype = "dashed", colour = "gray50") +
    # Plot individuals with missing lengths and weights
    geom_point(data = filter(lengths.sub, missing.length == TRUE), 
...
Total lengths (or mantle length, for Market Squid) vs. weight for all speciments, symbolized by cruise leg.  
...
lw.plot.leg <- ggplot() +
  ...
  # Plot seasonal length models for each species
  geom_line(data = lw.df, aes(TL, W), linetype = 'dashed', colour = "gray50", alpha = 0.75) +
  # Facet by species
  facet_wrap(~scientificName, scales = "free") +
  # Format plot
