...
haul.all <- haul.all %>% 
  # Remove bad trawls
  filter(haul, tolower(trawlPerformance) %in% trawl.performance) %>% 
...
haul.paths <- select(haul.all, haul, lat = startLatDecimal, long = startLongDecimal) %>% 
  ...
  mutate(
    ...
    popup    = paste('<b>Haul:', haul, '</b><br/>',
                     ...
                     'All CPS:', AllCPS, 'kg')) %>% 
  left_join(select(haul.all, haul, 
...
if (file.exists(here("Data/Nav/nav_data.Rdata"))) {
  ...
  #   st_cast("LINESTRING")
  # 
  # # Map haul paths with vessel nav
  # mapview(nav.sf, color = "#747C8A", alpha = 0.5, layer.name = "Vessel Track", legend = FALSE) +
  #   mapview(haul.paths, color = "black", layer.name = "Haul Paths", legend = FALSE)
  ...
  leaflet() %>% 
    ...
    # Add nav data
    addPolylines(data = nav.legs, color = "#000414", weight = 1, 
