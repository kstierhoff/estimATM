...
# Filter catch data
  ...
  left_join(select(haul.all, haul, cluster)) %>% 
...
#     missing.length = case_when(
#       is.na(totalLength_mm) & !scientificName == "Doryteuthis (Loligo) opalescens" ~ T,
#       is.na(mantleLength_mm) & scientificName == "Doryteuthis (Loligo) opalescens" ~ T,
#       TRUE ~ F)) %>% 
#   mutate(totalLength_mm = round(totalLength_mm),
#          mantleLength_mm = round(mantleLength_mm))
...
# Real length/weight relationship models for each species
...
lengths <- lengths %>% 
  mutate(key = paste(collection, species, specimenNumber)) %>% 
  left_join(outliers) %>% 
  left_join(select(haul.all, haul, datetime = equilibriumTime)) %>% 
