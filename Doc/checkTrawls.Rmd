...
lengths.all <- lengths.all %>% 
  ...
  mutate(
    missing.length = case_when(
      ...
      is.na(forkLength_mm) & scientificName %in% c("Scomber japonicus", "Trachurus symmetricus", 
                                                   "Clupea pallasii", "Etrumeus acuminatus") ~ TRUE,
      is.na(totalLength_mm) & scientificName %in% c("Merluccius productus") ~ TRUE,
      is.na(mantleLength_mm) & scientificName %in% c("Doryteuthis (Loligo) opalescens") ~ TRUE,
      TRUE ~ NA),
  ...
  mutate(nativeLength_mm = case_when(
    ...
    scientificName %in% c("Merluccius productus") ~ totalLength_mm,
...
lengths <- lengths.all %>% 
  filter(cruise %in% cruise.name & ship %in% cruise.ship & 
           scientificName %in% c(cps.spp, "Doryteuthis (Loligo) opalescens", "Merluccius productus") & 
           toupper(isRandomSample) == "Y") 
...
plot.deployment <- ggplot() +
  geom_line(data = haul, aes(haul, as.numeric(deploymentTime), color = "Deployment")) +
  geom_point(data = haul, aes(haul, as.numeric(deploymentTime)), shape = 21, fill = "white", colour = "blue") +
  geom_line(data = haul, aes(haul, as.numeric(recoveryTime), color = "Recovery")) +
  geom_point(data = haul, aes(haul, as.numeric(recoveryTime)), shape = 21, fill = "white", colour = "red") +
  geom_line(data = haul, aes(haul, as.numeric(duration), color = "Fishing")) +
  ...
  labs(x = "Haul", y = "Duration (mins)", title = "Deployment/Fishing/Recovery Time", color = NULL) +
...
# ggplotly(plot.duration.all, dynamicTicks = TRUE) 
```
