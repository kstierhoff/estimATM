...
if (get.db) {
  if (trawl.source == "SQL") {
    ...
    haul.db <- dir_ls(file.path(survey.dir[survey.vessel.primary],
...
if (nrow(catch) > 0) {
  ...
  if (!"Engraulis mordax" %in% names(haul.summ.wt))            {haul.summ.wt$`Engraulis mordax`      <- 0}
  if (!"Sardinops sagax" %in% names(haul.summ.wt))             {haul.summ.wt$`Sardinops sagax`       <- 0}
  if (!"Scomber japonicus" %in% names(haul.summ.wt))           {haul.summ.wt$`Scomber japonicus`     <- 0}
  if (!"Trachurus symmetricus" %in% names(haul.summ.wt))       {haul.summ.wt$`Trachurus symmetricus` <- 0}
  if (!"Clupea pallasii" %in% names(haul.summ.wt))             {haul.summ.wt$`Clupea pallasii`       <- 0}
  if (!"Atherinopsis californiensis" %in% names(haul.summ.wt)) {haul.summ.wt$`Atherinopsis californiensis` <- 0}
  if (!"Etrumeus acuminatus" %in% names(haul.summ.wt))         {haul.summ.wt$`Etrumeus acuminatus` <- 0}
...
# Two alternative methods for selecting columns; leaving until testing is completed.
...
# 
# catch.pie <- cluster.summ.wt %>% 
#   select(cluster, long, lat, any_of(c("Anchovy", "JackMack", 
#          "Jacksmelt", "PacHerring", "PacMack", "RndHerring", 
#          "Sardine", "AllCPS"))) %>% 
#   st_as_sf(coords = c("long","lat"), crs = crs.geog)
...
lengths.all <- lengths.all %>% 
  ...
  mutate(
    missing.length = case_when(
      is.na(standardLength_mm) & scientificName %in% c("Sardinops sagax", "Engraulis mordax") ~
        TRUE,
      is.na(forkLength_mm) & scientificName %in% c("Scomber japonicus", "Trachurus symmetricus", "Clupea pallasii", "Etrumeus acuminatus") ~
        TRUE,
  ...
  mutate(nativeLength_mm = case_when(
    ...
    scientificName %in% c("Scomber japonicus", "Trachurus symmetricus", 
                          "Clupea pallasii", "Etrumeus acuminatus") ~ forkLength_mm,
...
Trawl durations are examined to look at trends in deployment, recovery, and total evolution times. The top panel shows total evolution time (from netOnDeckTime - netInWaterTime; green), while the bottom panel shows the deployment time (equilibriumTime - netInWaterTime, blue), fishing time (haulBackTime - equilbriumTime), and recovery time (netOnDeckTime - haulBackTime, red).

```{r check-trawl-duration-all}
# Plot trawl durations
plot.evolution <- ggplot() + 
  geom_line(data = haul, aes(haul, as.numeric(evolutionTime)), colour = "green") +
  geom_point(data = haul, aes(haul, as.numeric(evolutionTime)), shape = 21, fill = "white", colour = "green") +
  labs(x = "Haul", y = "Duration (mins)", title = "Evolution Time")

plot.deployment <- ggplot() + 
  geom_line(data = haul, aes(haul, as.numeric(deploymentTime)), colour = "blue") +
  geom_point(data = haul, aes(haul, as.numeric(deploymentTime)), shape = 21, fill = "white", colour = "blue") +
  geom_line(data = haul, aes(haul, as.numeric(recoveryTime)), colour = "red") +
  geom_point(data = haul, aes(haul, as.numeric(recoveryTime)), shape = 21, fill = "white", colour = "red") +
  geom_line(data = haul, aes(haul, as.numeric(duration)), colour = "black") +
  geom_point(data = haul, aes(haul, as.numeric(duration)), shape = 21, fill = "white", colour = "black") +
  labs(x = "Haul", y = "Duration (mins)", title = "Deployment/Fishing/Recovery Time")
