...
pacman::p_load(tidyverse,plotly,lubridate,mapview,kableExtra,
               here,bookdown,odbc,DBI,knitr,DT,fs,sf)
...
# Get haul starts
haul.starts <- select(haul.all, haul, startLatDecimal, startLongDecimal)
names(haul.starts) <- c("haul","lat","long")

# Get haul ends
haul.ends <- select(haul.all, haul, stopLatDecimal, stopLongDecimal)
...
Trawl distances are examined to look for data entry errors. Trawls longer than 6 nmi are highlighted orange.
...
haul.paths.sf %>% 
  ...
  mutate(distance = cell_spec(
    ...
    background = ifelse(distance <= 6, "white", "orange"))) %>% 
