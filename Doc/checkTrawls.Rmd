...
Note: log(Total Weight) is presented given large total catches in some hauls.  
...
# Plot catch 
catch.bar.plot.all <- ggplot(catch.bar, aes(factor(haul), log(totalWeight+1), fill = group)) +
...
ggsave(catch.bar.plot.all, file = here("Figs/fig_catch_weight_leg.png"),
       width = 12, height = 8)
...
# Plot catch 
catch.bar.plot.cps <- ggplot(catch.bar.cps, aes(factor(haul), log(totalWeight + 1), fill = group)) +
  geom_bar(colour = "black", position = "stack", stat = "identity") + 
  facet_wrap(~leg, scales = "free", ncol = 1) +
  scale_fill_manual(name = 'Species',
...
ggsave(catch.bar.plot.cps, file = here("Figs/fig_catch_weight_leg_cps.png"),
       width = 12, height = 8)
