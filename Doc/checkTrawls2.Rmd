...
pacman::p_load(tidyverse,plotly,lubridate,mapview,kableExtra,leaflet,
               here,bookdown,odbc,DBI,knitr,DT,fs,sf,atm,reshape2)
...
# Select only hauls from current survey
haul <- haul.all %>% 
...
if (trawl.source == "Access") {
  ...
  # haul.paths2 <- haul.paths %>% 
  #   mutate(
  #     startLong = as.data.frame(st_coordinates(.))$X[ , 1],
  #     startLat  = as.data.frame(st_coordinates(.))$Y[ , 1],
  #     endLong   = as.data.frame(st_coordinates(.))$X[ , 2],
  #     endLat    = as.data.frame(st_coordinates(.))$Y[ , 2])
}

# Convert haul midpoints to sf
...
lengths.all <- lengths.all  %>% 
  ...
  left_join(select(haul, cruise, ship, haul, season)) %>% 
  filter(scientificName %in% c(cps.spp, "Doryteuthis (Loligo) opalescens"))
...
lengths.all  <- lengths.all %>%
  mutate(
    totalLength_mm = case_when(
      scientificName == "Clupea pallasii"       ~ convert_length("Clupea pallasii", forkLength_mm, "FL", "TL"),
      scientificName == "Engraulis mordax"      ~ convert_length("Engraulis mordax", standardLength_mm, "SL", "TL"),
      scientificName == "Sardinops sagax"       ~ convert_length("Sardinops sagax", standardLength_mm, "SL", "TL"),
      scientificName == "Scomber japonicus"     ~ convert_length("Scomber japonicus", forkLength_mm, "FL", "TL"),
      scientificName == "Trachurus symmetricus" ~ convert_length("Trachurus symmetricus", forkLength_mm, "FL", "TL"),
...
# ggplot(lengths.all, aes(totalLength_mm, weightg)) +
...
test.df <- data.frame(scientificName = c("Engraulis mordax","Sardinops sagax","Doryteuthis opalescens"),
                      totalLength_mm = c(20,20,20),
                      weightg = c(NA, NA, 3))

test.df2 <- test.df %>% 
  mutate(est.wt = case_when(
    is.na(weightg) ~ estimate_weight(scientificName, totalLength_mm, season = "summer"),
    TRUE ~ weightg
  )) 
...
lengths <- lengths.all %>% 
  ...
  filter(cruise %in% cruise.name & ship %in% cruise.ship & 
           scientificName %in% c(cps.spp, "Doryteuthis (Loligo) opalescens") & 
           toupper(isRandomSample) == "Y") #%>%   filter(!is.na(K))
...
# Select only specimens with missing length AND weight
bad.lengths %>% 
  select(haul, collection, scientificName, specimenNumber, SL = standardLength_mm, 
         FL = forkLength_mm, TL = totalLength_mm, ML = mantleLength_mm, 
         weightg, sex, K, flaggedData) %>%
...
```{r missing-sex}
# Select only specimens not classified as male, female, or unknown
lengths %>% 
  filter(sex == "missing") %>% 
  select(haul, collection, scientificName, specimenNumber, SL = standardLength_mm, 
         FL = forkLength_mm, TL = totalLength_mm, ML = mantleLength_mm, 
...
if (nrow(lengths.sub) > 0) {
  ...
  lw.plot <- ggplot() + 
    ...
    geom_smooth(data = lengths.sub, aes(mantleLength_mm, weightg), se = FALSE, 
                color = "black", alpha = 0.5, linetype = "dashed", size = 0.5) +
...
if (trawl.source == "Access") {
  ...
  haul.bad %>% 
    mutate(trawlDate = date(wireOutTime)) %>%
    select(haul, trawlDate, orderOcc, trawlPerformance) %>% 
    datatable(rownames = FALSE)  
}
```
...
catch.comp <- left_join(select(catch.all, haul, scientificName, subSampleWtkg, subSampleCount),
