---
title: "Acoustic Line Kilometers"
...
if (get.nav.ns) {
  ...
  save(nav.ns, nav.ns.sf, file = here("Data/Nav/nav_data_nearshore.Rdata"))
...
if (get.nav.sd) {
  ...
  if (file.exists(here("Data/Nav/nav_data_saildrone.Rdata"))) {
    ...
    nav.lag.sd <- difftime(now(tzone = "UTC"), max(ymd_hms(nav.sd$time)), units = "hours")
  ...
  if (get.nav) {
    ...
    nav.now.sd <- nav.sd.sf %>% 
      group_by(saildrone) %>% 
      slice(n()) %>% 
      mutate(label = paste("Saildrone", saildrone, "Last position:", time, "UTC"),
...
if (exists("nav.ns")) {
  ...
  if (extract.bathy) {
    ...
    nav.depth.ns <- get.depth(noaa.bathy, nav.ns$long, nav.ns$lat, locator = F, distance = T) %>%
      rename(long = lon) %>%
      bind_cols(select(nav.ns, time, dist)) %>%
      mutate(dist.depth = c(0, diff(dist.km))) %>%
      filter(dist.depth < 100)  %>% # Remove distant points
      mutate(depth_bin = cut(depth, c(min(depth), -200, 0), include.lowest = T, labels = F),
