...
if (trawl.source == "Access") {
  # The Access database format changed in 2022, so the steps below handle that format
  if (year(haul.all$netInWaterTime[1]) >= 2021) {
    ...
    # Process using the >= 2022 Access format
    haul.all <- haul.all %>% 
      arrange(haul) %>% 
      mutate(
        startLatDecimal  =   startLatitudeDegrees + (startLatitudeMinutes/60),
  ...
  # Process the SQL data, which hasn't changed over time
  haul.all <- haul.all %>% 
    arrange(haul) %>% 
    mutate(
...
if (save.figs) {
  if (exists("nasc.map.cps") & exists("cufes.density.all") & exists("trawl.pie.haul.wt")) {
    ...
    save(nasc.trawl.acoustic.prop,
         file = here("Output/nasc_prop_map.Rdata"))
...
# Create sf used to remove nearshore portions of the core area tranects ---------------------------
