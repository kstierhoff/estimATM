...
if (save.figs) {
  ...
  trawl.catch.plot <- base.map +
    # Plot transects data
    geom_point(data = nasc, aes(X, Y, group = transect.name),
...
# Format CUFES data
cufes <- cufes.all %>% 
  select(SampleNumber, Year, Ship, Cruise, lat = StartLatitude, long = StartLongitude, Duration, 
...
if (save.figs) {
  ...
  cufes.density.all <- base.map +
    ...
    geom_point(data = cufes.neg,
               aes(X, Y), shape = 3, size = 0.5, colour = 'black', alpha = 0.5) +
    # Plot only positive cufes samples
    geom_point(data = filter(cufes, Density > 0, Species %in% cufes.plot.spp),
