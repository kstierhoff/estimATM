...
CPS catch in each trawl cluster is mapped by a) proportion of total weight, b) acoustic proportion by number, and c) acoustic proportion by weight; if done correctly, panels b) and c) should be the same.  
...
if (save.figs) {
  ...
  acoustic.pie.plot.comp <- plot_grid(trawl.catch.plot, acoustic.pie.plot.indiv, acoustic.pie.plot.wg,
                                     nrow = 1, labels = c("a)", "b)", "c)"))
...
if (do.bootstrap) {
  ...
  # abundance.estimates[atm:::is.nan.df(abundance.estimates)] <- 0
...
if (do.bootstrap) {
  ...
  for (i in unique(strata.nearshore$scientificName)) {
    ...
    for (j in unique(nasc.nearshore$stratum)) {
      ...
      # Calculate biomass using bootstrap function ----
      set.seed(1) # Set seed for repeatable results
      boot.df <- estimate_bootstrap(nasc.nearshore, cluster.final[[i]], j, 
                                    stratum.area = stratum.area, 
                                    species = i, do.lf = do.lf, 
                                    boot.number = boot.num)$data.frame
      # Extract biomass estimates; remove first (point) estimate
...
if (save.figs) {
  source(here("Code", paste0("map_nearshore_biomass_", survey.name, ".R")))  
}
...
```{r calibration-table-primary,eval=FALSE}
