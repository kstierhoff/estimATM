...
# Often or usually F
copy.bib          <- T # Copy bibliography files (requires connection to AST2)
...
if (copy.files) {
  ...
  for (i in nasc.vessels) {
    if (dir.exists(file.path(survey.dir[i], nasc.dir[i]))) {
      # List CSV files on survey directory
      nasc.files <- dir_ls(file.path(survey.dir[i], nasc.dir[i]),
                           regex = nasc.pattern.cps[i])
      # Copy CSV files to local directory
      file_copy(nasc.files, here("Data/Backscatter", i), overwrite = overwrite.csv)  
    }
  } 
   
  # Copy CUFES files -------------------------------------------------------
  if (cufes.source == "SQLite") {
    file_copy(file.path(cufes.dir.sqlite, cufes.db.sqlite), here("Data/CUFES"),
              overwrite = overwrite.files)
  }
  
  # Copy Access database -------------------------------------------------------
  if (trawl.source == "Access") {
    file_copy(file.path(trawl.dir.access, trawl.db.access),here("Data/Trawl"),
              overwrite = overwrite.files)
  }
}
```  
