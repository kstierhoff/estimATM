...
For each vessel, plot the backscatter from the CTD.app (`cps.NASC`, or equivalent) to backscatter from the default fixed integration depth (`NASC.70`).

```{r compare-cps-nasc}
# Compare cps.nasc to NASC.70
ggplot(nasc, aes(NASC.70, cps.nasc)) +
  geom_point() +
...
landmarks <- data.frame(
  ...
  lat.start = c(stock.break.sar, stock.break.anch, 48.490),
  y = rep(0,3))
...
if (nrow(strata.summ.rm) > 0) {
  ...
  biomass.dens.plot.lat <- ggplot(strata.summ) + 
    ...
    geom_vline(xintercept = c(stock.break.sar, stock.break.anch, 48.490), linetype = "dashed") +
