...
pacman::p_load(tidyverse,swfscMisc,reshape2,tcltk,lubridate,sp,rgeos,
          ...
          rworldmap,rworldxtra,data.table,xts,stplanr,ggspatial)
...
survey.plan <- base.map +    
  ...
  scale_linetype_manual("Type", values = c(Adaptive = "dashed", Compulsory = "solid",
                                           Offshore = "solid", Nearshore = "solid",
                                           Transit = "dashed")) +
...
if (process.csv) {
  ...
  if (process.csv.all) {
    ...
    load(here("Data/Backscatter/nasc_cps.Rdata"))
  }
    ...
    if (length(nasc.files) > 0) {
      ...
      if (rm.offshore[i]) {
        ...
        nasc.vessel <- nasc.vessel %>% 
          ...
          filter(!str_detect(transect.orig, "\\d{3}O"))  
      }
  ...
  save(nasc, file = here("Data/Backscatter/nasc_cps.Rdata"))
} else {
  load(here("Data/Backscatter/nasc_cps.Rdata"))
}
...
nasc <- left_join(select(nasc, -transect), select(tx.order, transect.name, transect))  
