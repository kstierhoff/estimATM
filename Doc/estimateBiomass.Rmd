    ...
    if (length(nasc.files) > 0) {
      ...
      if (rm.offshore[i]) {
        nasc.offshore <- nasc.vessel %>% 
          filter(str_detect(transect.orig, "\\d{3}O") | str_detect(tolower(filename), "offshore")) 
...
```{r filter-saildrone-nav,eval=FALSE}
...
# Conver cluster midpoints to projected points
  ...
  for (i in unique(strata.primary$scientificName)) {
    for (j in unique(filter(strata.primary, scientificName == i)$stock)) {
      ...
      biomass.dens <- base.map +
        ...
        scale_colour_discrete('Stratum') + 
        # Plot vessel track
        geom_sf(data = nav_sf, colour = 'gray50', size = 0.25, alpha = 0.5) +
        # Plot zero nasc data
