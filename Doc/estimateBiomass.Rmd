...
# Remove purse seine samples from the clf data frame; will be added from an 
# external file later
# if (survey.name == "2207RL") {
...
clf <- clf %>% 
  mutate(sample.type = "Trawl",
         adjusted = as.logical(FALSE))

# Add adjusted seine catches from LM in 2022 here
if (survey.name == "2207RL") {
  clf.lm.2022 <- read_csv(here("Data/Seine/lm_clusters_2022_adjusted.csv")) %>% 
    # Remove duplicate Lasker clusters
    # Remove LM and LBC clusters S of Cape Mendocino
    filter(cluster > 32,
           lat >= 40.42) %>% 
    mutate(sample.type = "Purse seine",
  ...
  # Replace NA's with 0's
  clf.lm.2022[is.na(clf.lm.2022)] <- 0
  
  
  clf.lm.2022 <- clf.lm.2022 %>% 
    ## Add prop and sigmas for "other" species
    mutate(
      prop.other = case_when(
        prop.sar != 0 & adjusted == TRUE ~ 1 - prop.sar,
        # prop.sar == 0 & adjusted == TRUE ~ 1,
        TRUE ~ prop.other),
      prop.other.wg = prop.other) %>% 
    mutate(
      sigmawg.other = case_when(
        prop.other == 0 ~ 1,
        TRUE ~ prop.other),
      sigmaindiv.other = sigmawg.other) %>% 
    select(-haul)
  
  # Combine clf files
  clf <- bind_rows(clf, clf.lm.2022)
  
  # Plot replacement clusters, symbolized by sample type and whether or not adjusted
  # ggplot(clf, aes(long, lat, colour = sample.type, fill = adjusted)) +
