...
if (process.csv) {
  ...
  for (i in nasc.vessels) {
    ...
    if (length(nasc.files) > 0) {
      ...
      # # Remove first and last interval from Lisa Marie data for 1907RL
      # if (survey.name == "1907RL" & i == "LM") {
      #   nasc.vessel2 <- nasc.vessel %>% 
...
(ref:rlf-jack) Standard length ($L_S$) frequency distributions of Jack Mackerel (_Trachurus symmetricus_) per nighttime trawl cluster, annotated with the number of individuals caught and their percentage contributions to the abundance in each stratum and across all strata (i.e., for the entire survey).
...
```{r rlf-her,out.height='7in',fig.pos='H'}
