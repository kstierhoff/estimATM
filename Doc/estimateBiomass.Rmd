...
if (get.nav) {
  ...
  if (nav.lag > 24) {
    ...
    nav.temp <- data.frame(read.csv(dataURL, header = FALSE, colClasses = erddap.classes, 
      ...
      if (nrow(nasc.vessel) > 0) {
        ...
        if (source.cps.nasc[i]) {
          ...
          if (survey.name %in% c("1507SH")) {
            ...
            # cps.nasc.temp$NASC.50.Juan <- cps.nasc.temp$NASC.50
            # 
            # nasc.vessel <- nasc.vessel %>% 
            #    left_join(select(cps.nasc.temp, key, NASC.50.Juan))
            # 
            # ggplot(nasc.vessel, aes(NASC.50, NASC.50.Juan)) + geom_point()
            # 
            # 
            # nasc.vessel <- nasc.vessel %>% 
            #   mutate(nasc.diff2 = NASC.50 - NASC.50.Juan)
...
if (survey.name == "1507SH") {
  ...
  saveRDS(lengths.all, file = here("Output/lengths_all_1507SH.rds"))
  
  lengths.all <- lengths.all %>% 
    mutate(totalLength_mm = case_when(
      scientificName == "Clupea pallasii" & cruise == "201507" & !is.na(standardLength_mm) ~ 
        convert_length("Clupea pallasii", .$standardLength_mm, "SL", "TL"),
      TRUE ~ totalLength_mm)) %>% 
    mutate(forkLength_mm = case_when(
      scientificName == "Clupea pallasii" & cruise == "201507" & is.na(forkLength_mm) ~ 
...
if (get.db) {
  ...
  cufes.all <- cufes.all %>%
    ...
    filter(between(Start, date(erddap.survey.start), date(erddap.survey.end))) %>% 
    project_df(to = crs.proj)
...
```{r interactive-data-explorer, fig.align='left', fig.cap='(ref:interactive-data-explorer)',echo=FALSE}
# Leaflet options
...
txPal    <- colorFactor(c("red","blue","orange","green", "yellow"), 
                        c("Adaptive","Compulsory","Nearshore","Offshore", "Transit"))
