...
if (copy.files) {
  ...
  for (d in nasc.vessels) {
    csv.files.cps <- dir_ls(file.path(survey.dir[d], 
                                      nasc.dir[d]), 
                            regex = nasc.pattern.cps[d])
    
    file_copy(csv.files.cps, here("Data/Backscatter", d), 
              overwrite = overwrite.csv)  
    
    # Copy CSV files for Krill
    csv.files.krill <- dir_ls(file.path(survey.dir[d], 
                                        nasc.dir[d]), 
                              regex = nasc.pattern.krill[d])
    
    file_copy(csv.files.krill, here("Data/Backscatter", d), 
              overwrite = overwrite.csv)
  }
  ...
  for (i in nasc.vessels) {
    ...
    if (length(nasc.files) > 0) {
      ...
      if (exists("processed.csv")) {
        processed.csv <- sort(c(processed.csv, processed.csv.vessel))
      } else {
...
cufes.colors.all     <- cufes.colors[which(names(cufes.colors) %in% unique(cufes$Species))]
cufes.spp.labels.all <- cufes.spp.labels[which(names(cufes.spp.labels) %in% unique(cufes$Species))]
...
if (save.figs) {
  # Create trawl figure
  acoustic.pie.map.indiv <- base.map +
    # Plot transects data
    geom_sf(data = transects.sf, size = 0.5, colour = "gray70", 
            alpha = 0.75, linetype = "dashed") +
    # plot ship track data
  ...
  acoustic.pie.plot.comp <- plot_grid(trawl.pie.cluster.wt, acoustic.pie.map.indiv, 
                                      acoustic.pie.map.wg,
                                      nrow = 1, labels = c("a)", "b)", "c)"))

  # Save trawl species proportions plots
  ggsave(acoustic.pie.plot.comp,
         filename = here("Figs/fig_trawl_proportion_comparison.png"),
         width = map.width*3, height = map.height)
...
include_graphics(here("Figs/fig_trawl_proportion_comparison.png"))
```
...
# Create trawl figure
acoustic.pie.map.final <- base.map +
  # Plot transects data
  geom_sf(data = transects.sf, size = 0.5, colour = "gray70", 
          alpha = 0.75, linetype = "dashed") +
  # plot ship track data
...
if (save.figs) {
  nasc.cufes.prop.plot      <- plot_grid(nasc.map, cufes.density.all, acoustic.pie.map.final,
                                     nrow = 1, labels = c("a)", "b)", "c)"))
  
  nasc.cufes.prop.plot.long <- plot_grid(nasc.map, cufes.density.all, acoustic.pie.map.final,
                                     ncol = 1, labels = c("a)", "b)", "c)"))
  # Save maps
...
(ref:biom-dens-anch-n-ns) Extrapolated nearshore biomass densities of northern stock of Northern Anchovy (_Engraulis mordax_), per strata, throughout the survey region. The blue numbers represent the locations of trawl clusters with at least one anchovy. The gray line represents the vessel track.

```{r biom-dens-anch-n-ns,fig.cap='(ref:biom-dens-anch-n-ns)',out.height='100%',fig.pos='H'}
...
(ref:biom-dens-anch-c-ns) Extrapolated nearshore biomass densities of central stock of Northern Anchovy (_Engraulis mordax_), per strata, throughout the survey region. The blue numbers represent the locations of trawl clusters with at least one anchovy. The gray line represents the vessel track.

```{r biom-dens-anch-c-ns,fig.cap='(ref:biom-dens-anch-c-ns)',out.height='100%',fig.pos='H'}
...
(ref:biom-dens-sar-n-ns) Extrapolated nearshore biomass densities of northern stock of Pacific Sardine (_Sardinops sagax_), per strata, throughout the survey region. The blue numbers represent the locations of trawl clusters with at least one sardine. The gray line represents the vessel track.

```{r biom-dens-sar-n-ns,fig.cap='(ref:biom-dens-sar-n-ns)',out.height='100%',fig.pos='H'}
...
(ref:biom-dens-sar-s-ns) Extrapolated nearshore biomass densities of southern stock of Pacific Sardine (_Sardinops sagax_), per strata, throughout the survey region. The blue numbers represent the locations of trawl clusters with at least one sardine. The gray line represents the vessel track.

```{r biom-dens-sar-s-ns,fig.cap='(ref:biom-dens-sar-s-ns)',out.height='100%',fig.pos='H'}
...
(ref:biom-dens-mack-ns) Extrapolated nearshore biomass densities of Pacific mackerel (_Scomber japonicus_), per strata, throughout the survey region. The blue numbers represent the locations of trawl clusters with at least one mackerel. The gray line represents the vessel track.

```{r biom-dens-mack-ns,fig.cap='(ref:biom-dens-mack-ns)',out.height='100%',fig.pos='H'}
...
(ref:biom-dens-jack-ns) Extrapolated nearshore biomass densities of jack mackerel (_Trachurus symmetricus_), per strata, throughout the survey region. The blue numbers represent the locations of trawl clusters with at least one jack mackerel. The gray line represents the vessel track.

```{r biom-dens-jack-ns,fig.cap='(ref:biom-dens-jack-ns)',out.height='100%',fig.pos='H'}
...
(ref:biom-dens-her-ns) Extrapolated nearshore biomass densities of Pacific herring (_Clupea pallasii_), per strata, throughout the survey region. The blue numbers represent the locations of trawl clusters with at least one herring. The gray line represents the vessel track.

```{r biom-dens-her-ns,fig.cap='(ref:biom-dens-her-ns)',out.height='100%',fig.pos='H'}
