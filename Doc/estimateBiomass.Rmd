...
if (survey.name %in% c("2207RL")) {
  # Get max haul and cluster for Lasker to increment 2021 Lasker data
  max.cluster.rl <- max(haul$cluster)
  max.haul.rl    <- max(haul$haul)
  
  # Get Lasker data from  
  ...
  # ggplot(haul, aes(startLongDecimal, startLatDecimal, colour = cruise)) +
  #   geom_text(aes(label = haul)) + 
  #   coord_map() + facet_wrap(~cruise)
}
...
if (survey.name %in% c("2207RL")) {
  # Get 2022 hauls to filter 2021 catch
  hauls.2022 <- haul$haul
