...
if (copy.files) {
  ...
  dir_create(here("Data", c("Backscatter", "CTD", "CUFES", "Trawl", "UCTD")))
    ...
    if (length(nasc.files) > 0) {
      ...
      save(nasc.vessel, 
           file = here("Data/Backscatter", i, 
                       paste("nasc_vessel_", i, "_RAW.Rdata", sep = "")))
      
      processed.csv.vessel <- unique(nasc.vessel$filename)
      ...
      if (rm.offshore[i]) {
        ...
        save(nasc.offshore, 
             file = here("Data/Backscatter", i, 
                         paste0("nasc_vessel_", i, "_offshore.Rdata")))
      ...
      if (rm.nearshore[i]) {
        ...
        save(nasc.nearshore, 
             file = here("Data/Backscatter", i, 
                         paste0("nasc_vessel_", i, "_nearshore.Rdata")))
      ...
      # Save processed CSV data from each survey vessel
      save(nasc.vessel, file = here("Data/Backscatter", i, 
                                    paste0("nasc_vessel_", i, ".Rdata")))
      ...
      if (exists("processed.csv")) {
        processed.csv <- c(processed.csv, processed.csv.vessel)
      } else {
...
# Save image
# save.image(here("Data/debug_img.Rdata"))
# load(here("Data/debug_img.Rdata"))
...
cluster.mid.proj <- project_sf(cluster.mid.sf, crs = crs.proj)
...
if (save.figs) {
  ...
  trawl.cluster.plot <- base.map +
    ...
    geom_point(data = cluster.mid.proj, aes(X, Y), colour = "green", shape = 21) +
    geom_shadowtext(
      data = cluster.mid.proj, aes(X, Y, label = cluster),
...
save(haul.summ.wt, cluster.summ.wt, file = here("Output/catch_summary_wt.Rdata"))
```
...
if (save.figs) {
  ...
  nasc.cluster.plot <- base.map +
    ...
    geom_text(data = cluster.mid.proj, aes(X, Y, label = cluster),
              colour = 'gray20', size = 2) +
    # Plot positive trawl cluster midpoints
    geom_shadowtext(data = filter(cluster.mid.proj, cluster %in% super.clusters$cluster), 
...
if (save.figs) {
  ...
  ggsave(nasc.trawl.plot,
         filename = here("Figs/fig_nasc_cluster_trawl_proportion.png"),
         width = map.width*2, height = map.height)
...
Survey transects performed aboard _`r survey.vessel`_ overlaid with proportions of CPS species in trawl clusters (black points indicate trawl clusters with no CPS). 
...
cufes <- cufes.all %>% 
  select(SampleNumber, Year, Ship, Cruise, StartLatitude, StartLongitude, Duration, 
  ...
  gather(Species, Counts, -SampleNumber, -Year, -Ship, -Cruise, 
         -StartLatitude, -StartLongitude, -Duration, -Comments) %>% 
  mutate(Density = Counts/Duration/0.64,
         # Create bins for defining point size in NASC plots
...
if (save.figs) {
  ...
  ggsave(nasc.cufes.trawl.plot.long, 
         filename = here("Figs/fig_nasc_cufes_trawl_long.png"),
...
include_graphics(here("Figs/fig_nasc_cufes_trawl_long.png"))
```
...
clf <- clf %>% 
  ...
  left_join(haul.summ.wt,   by = 'cluster') %>% 
...
if (save.figs) {
  ...
  ggsave(acoustic.pie.plot.comp,
         filename = here("Figs/fig_trawl_acoustic_proportion_comparison.png"),
         width = map.width*2, height = map.height)
...
include_graphics(here("Figs/fig_trawl_acoustic_proportion_comparison.png"))
```
...
if (save.figs) {
  ...
  ggsave(nasc.cufes.prop.plot, 
         filename = here("Figs/fig_nasc_cufes_trawl_proportion.png"),
         width = map.width*3, height = map.height)
  
  ggsave(nasc.cufes.prop.plot.long, 
         filename = here("Figs/fig_nasc_cufes_trawl_proportion_long.png"),
         width = map.width, height = map.height*3)
...
include_graphics(here("Figs/fig_nasc_cufes_trawl_proportion_long.png"))
```
...
if (save.figs) {
  ...
  # Save figure
  ggsave(here("Figs/fig_acoustic_strata.png"), map.stratum.all,
