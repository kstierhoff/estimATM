...
if ("segments" %in% ls()) rm("segments")

# Segmentize allowing for different spacing in each loc
for (i in baselines$loc) {
  segments.tmp <- baselines %>% 
    filter(loc == i) %>% 
    st_segmentize(., dfMaxLength = 1852 * tx.spacing.fsv[i])
  
  if ("segments" %in% ls()) {
    segments <- rbind(segments, segments.tmp)  
  } else {
