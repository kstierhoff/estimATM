...
if (sampling.trawl) {
  ...
  if (scale.pies) {
    haul.pie$radius    <- pie.radius*log(haul.pie$bin.level+1)
    cluster.pie$radius <- pie.radius*log(cluster.pie$bin.level+1)
  } else {
...
if (sampling.acoustics) {
  if (process.csv) {
    ...
    if (process.csv.all) {
      ...
      if (exists("processed.cps")) {
        ...
        csv.files.cps <- csv.files.cps[!fs::path_file(csv.files.cps) %in% processed.cps]  
      }
    }
  ...
  nasc.cps.sf <- nasc.cps %>%
    filter(lat != 999, long != 999) %>%
    select(filename, transect, int, dist_m, datetime, lat, long, cps.nasc = NASC.100) %>% 
  ...
  processed.cps <- unique(fs::path_file(nasc.cps$filename))
...
i.map <- leaflet() %>% 
  ...
  hideGroup(c("Backscatter (Large)", "Backscatter-Krill", "Backscatter-Krill (Small)", 
              "CUFES (Negative)", "CUFES Egg Density-Squid", 
              "Vessel Positions", "Saildrone Tracks", 
              "Planned Transects (Ancillary)", "Saildrone Tracks", "Saildrone Positions")) %>% 
...
# save(i.map, file = here("Output/imap/plotBio_explorer.Rdata"))
# 
# htmlwidgets::saveWidget(i.map, here("Output/imap/imap_widget.html"),
#                         selfcontained = TRUE)
