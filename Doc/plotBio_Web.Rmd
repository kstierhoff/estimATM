...
dir_create(here(c("Data", "Doc","Figs","Images","Output","References")))
```
  ...
  nav <- nav %>% 
    mutate(long     = long - 360,
           ...
           leg      = paste("Leg", cut(as.numeric(date(datetime)), 
                                  leg.breaks, labels = F, include.lowest = T))) %>%
...
wpts <- vroom(here("Data/Nav", paste0("waypoints_", prj.name, ".csv"))) %>% 
  mutate(Group = paste(Type, Transect))
...
# Create transects sf
tx_sf <- st_as_sf(wpts, coords = c("Longitude","Latitude"), crs = crs.geog) %>% 
  group_by(Group) %>% 
  summarise(do_union = F) %>% 
  st_cast("LINESTRING") %>% 
