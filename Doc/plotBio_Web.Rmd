...
pacman::p_load(tidyverse,raster,cowplot,DBI,odbc,RSQLite,scatterpie,ggspatial,
               reshape2,readxl,lubridate,rnaturalearth,sf,rgeos,here,naniar,vroom,
...
if (copy.files) {
  ...
  haul.db <- list.files(file.path(survey.dir[survey.vessel.primary], 
                                  "DATA/BIOLOGICAL/HAUL"),
                        pattern = "TrawlDataEntry*.*accdb", full.names = T)
  file.copy(haul.db, here("Data/Trawl"), overwrite = overwrite.files)
  
  # Copy CUFES files
  cufes.file <- list.files(file.path(survey.dir[survey.vessel.primary], "DATA/BIOLOGICAL/CUFES"), 
                           pattern = "*.sqlite", full.names = T)
  file.copy(cufes.file, here("Data/CUFES"), overwrite = overwrite.files)
  
  # Copy CSV files
  csv.files.cps <- list.files(file.path(survey.dir[survey.vessel.primary], "PROCESSED/EV/CSV"), 
                          pattern = "*Final 38 kHz CPS.csv", full.names = T)
  file.copy(csv.files.cps, here("Data/Backscatter"), overwrite = overwrite.csv)
  
  # Copy CSV files
  csv.files.krill <- list.files(file.path(survey.dir[survey.vessel.primary], "PROCESSED/EV/CSV"), 
                          pattern = "*Z Final Krill 120.csv", full.names = T)
  file.copy(csv.files.krill, here("Data/Backscatter"), overwrite = overwrite.csv)
}
```
...
# Read planned waypoints
wpts <- vroom(here("Data/Nav", paste0("waypoints_", prj.name, ".csv")))
...
```{r process-csv-cps}
# RESUME HERE
# Incorporate CSV processing code from biomassEstimate (fread; but consider vroom)
