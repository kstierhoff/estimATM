...
cufes.neg <- filter(cufes.raw, AllEggs == 0, !is.na(Duration)) %>% 
  mutate(bin.level = 1) %>% 
  st_as_sf(coords = c("StartLongitude", "StartLatitude"), crs = crs.geog)

# Project negative CUFES samples
cufes.neg.proj <- project_sf(cufes.neg, crs = crs.proj) 

# Define plot levels for all species
cufes.levels.all <- cufes.proj %>% distinct(bin.level) %>% arrange()
cufes.labels.all <- cufes.labels[cufes.levels.all$bin.level]
cufes.sizes.all  <- cufes.sizes[cufes.levels.all$bin.level]
...
if (process.csv) {
  ...
  csv.files.cps <- dir_ls(here("Data/Backscatter"), 
                              glob = nasc.pattern.cps[survey.vessel.primary])
  ...
  if (length(csv.files.cps) > 0) {
    ...
    for (i in seq_along(csv.files.cps)) {
      # Extract backscatter from i-th file and combine results
...
# Average NASC.50 data over new intervals or number of intervals in a 2 km radius
...
nasc.plot.cps <- nasc.cps %>%
  select(filename, transect, int, lat, long, cps.nasc = NASC.50) %>% 
...
if (process.csv) {
  ...
  # List local CSV files
  csv.files.krill <- dir_ls(here("Data/Backscatter"), 
                          glob = nasc.pattern.krill[survey.vessel.primary])
  ...
  if (length(csv.files.krill) > 0) {
    ...
    # Process all .CSV files
    for (i in seq_along(csv.files.krill)) {
      # Extract backscatter from i-th file and combine results
...
wpts.sf <- wpts %>% 
  filter(Type %in% c("Compulsory","Adaptive","Nearshore","Offshore")) %>% 
...
txPal    <- colorFactor(c("red","blue","green","orange"), 
                        c("Adaptive","Compulsory","Offshore","Nearshore"))
...
i.map <- leaflet() %>% 
  ...
  addPolylines(data = tx_sf, color = ~txPal(Type), weight = 3, 
