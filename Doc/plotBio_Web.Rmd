...
pacman::p_load(tidyverse,raster,cowplot,DBI,odbc,RSQLite,scatterpie,ggspatial,
               reshape2,readxl,lubridate,rnaturalearth,sf,rgeos,here,naniar,
               shadowtext,knitr,ggrepel,fs,plotly,leaflet,leaflet.extras,bookdown,
               leaflet.minicharts,maptools,htmltools,mapview,DT,rworldmap,rworldxtra)
...
overwrite.files <- F # Overwrite existing files when copying (not CSV, see below)
process.csv     <- F # Process acoustic backscatter files

# Control script behavior (often F)
get.map         <- F # Import land simple features
resize.map      <- F # Resize map during survey; if T, uses anticipated bounds of survey area
overwrite.csv   <- F # Overwrite existing Echoview CSV files when copying
process.csv.all <- F # Process all CSV files (F = new files only)
add.mammal.data <- F # Add marine mammal observations
...
if (copy.files) {
  # Copy <- report files (.xlsx)
  haul.report <- list.files(file.path(survey.dir, "DATA/BIOLOGICAL/HAUL"), 
                            pattern = "HaulReport*.*xlsx", full.names = T)
  file.copy(haul.report, here("Data/HAUL"), overwrite = overwrite.files)
  
  # Copy trawl Access database (.accdb)
  haul.db <- list.files(file.path(survey.dir, "DATA/BIOLOGICAL/HAUL"), 
                            pattern = "TrawlDataEntry*.*accdb", full.names = T)
  file.copy(haul.db, here("Data/HAUL"), overwrite = overwrite.files)
  
  # Copy CUFES files
  cufes.file <- list.files(file.path(survey.dir, "DATA/BIOLOGICAL/CUFES"), 
                           pattern = "*.sqlite", full.names = T)
  file.copy(cufes.file, here("Data/CUFES"), overwrite = overwrite.files)
  
  # Copy CSV files
  csv.files.cps <- list.files(file.path(survey.dir, "PROCESSED/EV/CSV"), 
                          pattern = "*Final 38 kHz CPS.csv", full.names = T)
  file.copy(csv.files.cps, here("Data/NASC"), overwrite = overwrite.csv)
  
  # Copy CSV files
  csv.files.krill <- list.files(file.path(survey.dir, "PROCESSED/EV/CSV"), 
                          pattern = "*Z Final Krill 120.csv", full.names = T)
  file.copy(csv.files.krill, here("Data/NASC"), overwrite = overwrite.csv)
}
```
...
if (get.nav) {
  ...
  # Save results
  save(nav, nav.sf, nav.legs, file = here("Data/nav_data.Rdata"))
} else {
  # Load nav data
  load(here("Data/nav_data.Rdata"))
}

wpts <- read_csv(here("Data/NAV/waypoints_all.csv"))
...
save(catch.all, haul.all, lengths.all, spp.codes, lengthFreq.all, file = here("Data/HAUL/trawl_data.Rdata"))
```  
...
# Get haul starts
haul.starts <- select(haul, haul, startLatDecimal, startLongDecimal)
names(haul.starts) <- c("haul","lat","long")

# Get haul ends
haul.ends <- select(haul, haul, stopLatDecimal, stopLongDecimal)
names(haul.ends) <- c("haul","lat","long")

# Create data frame for plotting haul paths
haul.paths <- arrange(bind_rows(haul.starts, haul.ends), haul)
...
catch.summ.wt <- dcast(dplyr::select(catch, haul, cluster, scientificName, totalWeight),
...
#   select(Haul = haul, long = startLongDecimal, lat = startLatDecimal,
...
cluster.pos <- filter(catch.pie, AllCPS > 0) %>% 
  replace(. == 0, 0.0000001) # Substitute very small value for species with zero catch, just for pie charts

# Filter for empty trawls
cluster.zero <- filter(catch.pie, AllCPS == 0)
```

```{r leaflet-minichart,eval=F}
tilesURL <- "http://server.arcgisonline.com/ArcGIS/rest/services/Canvas/World_Light_Gray_Base/MapServer/tile/{z}/{y}/{x}"

basemap <- leaflet(width = "100%", height = "1000px") %>%
  addTiles(tilesURL)

colors <- c(anchovy.color, jack.mack.color, jacksmelt.color,
            pac.herring.color, pac.mack.color, sardine.color)

basemap %>%
  addMinicharts(
    catch.pie$long, catch.pie$lat,
    type = "pie",
    chartdata = catch.pie[, c("Anchovy", "JackMack", "Jacksmelt","PacHerring",
                             "PacMack","Sardine")], 
    colorPalette = colors, 
    width = 60 * sqrt(catch.pie$AllCPS) / sqrt(max(catch.pie$AllCPS)), transitionTime = 0,
    group = "Trawl Species Composition"
  )
...
```{r process-csv-cps}
if (process.csv) {
  if (length(dir_ls(here("Output"), regexp = "processed_cps.Rdata")) > 0) {
    # Load already processed CSV files
    load(here("Output/processed_cps.Rdata"))
  }
  
  # List local CSV files
  csv.files.cps <- list.files(here("Data/NASC"), 
  ...
  if (length(csv.files.cps) > 0) {
    ...
    for (i in 1:length(csv.files.cps)) {
      # Read i-th file
      nasc.csv <- read_csv(here("Data/NASC", csv.files.cps[i]))
      # Extract file name from path
      file.name <- csv.files.cps[i]
...
```{r process-csv-krill}
if (process.csv) {
  if (length(dir_ls(here("Output"), regexp = "processed_krill.Rdata")) > 0) {
    # Load already processed CSV files
    load(here("Output/processed_krill.Rdata"))
  }
  
  # List local CSV files
  csv.files.krill <- list.files(here("Data/NASC"), pattern = "*Z Final Krill 120.csv")
  ...
  if (length(csv.files.krill) > 0) {
    ...
    for (i in 1:length(csv.files.krill)) {
      # Read i-th file
      nasc.csv <- read_csv(here("Data/NASC", csv.files.krill[i]))
      # Extract file name from path
      file.name <- csv.files.krill[i]
      # Extract transect name from the file name
      transect <- unlist(strsplit(file.name, split = "_"))[2]
      # Summarize NASC by interval
      nasc.temp <- nasc.csv %>% 
...
```{r process-mammal-data}
if (add.mammal.data) {
  # Define breaks for P. whitesided dolphin group size
  pws.breaks       <- c(0, 50, 100, 200, 500, 1000, 10000) # for legend breaks
  pws.labels       <- c("<50", "50-100", "100-200", "200-500", "500-1000", ">1000") # for legend labels
  pws.sizes        <- c(1, 2, 3, 4, 5, 6) # for legend sizes
  
  # Define breaks for P. whitesided dolphin group size
  hb.breaks       <- c(0, 1, 2, 4, 8, 200) # for legend breaks
  hb.labels       <- c("1", "2", "3-4", "4-8", ">8") # for legend labels
  hb.sizes        <- c(1, 2, 3, 4, 5) # for legend sizes
  
  # Import and process humpback whale data
  hb.data <- read_csv(here("Data/Mammal/humpbacks.csv")) %>% 
    filter(!is.na(Lat), !is.na(Lon), !is.na(GroupSize)) %>%
    st_as_sf(coords = c("Lon","Lat"), crs = crs.geog) %>% 
    mutate(bin = cut(GroupSize, hb.breaks, include.lowest = T),
           bin.level = as.numeric(bin),
           label = as.character(bin.level),
           popup = label)
  
  # Import and process P. whitesided dolphin data
  pws.data <- read_csv(here("Data/Mammal/pws_dolphins.csv")) %>%
    filter(!is.na(Lat), !is.na(Lon), !is.na(GroupSize)) %>%
    st_as_sf(coords = c("Lon","Lat"), crs = crs.geog) %>% 
    mutate(bin = cut(GroupSize, pws.breaks, include.lowest = T),
           bin.level = as.numeric(bin),
           label = as.character(bin.level),
           popup = label)
  
  # Project PWS and humpback data
  pws.data.proj <- project_sf(pws.data, crs.proj)
  hb.data.proj  <- project_sf(hb.data, crs.proj)
}
```

```{r process-seeps}
# Read seep regions
seeps <- read_csv(here("Data/Seeps/seeps_acoustics.csv")) %>% 
  mutate(
    Type = case_when(
...
# Create transects sf
tx_sf <- st_as_sf(wpt.all, coords = c("long","lat"), crs = crs.geog) %>% 
  group_by(type, line) %>% 
  summarise(do_union = F) %>% 
  st_cast("LINESTRING") %>% 
  filter(type != "extra")
...
annotation.lon <- min(range(nasc.cps$long))
annotation.lat <- max(range(nasc.cps$lat))
...
if (save.figs) {
  ...
  if (add.mammal.data) {
    ...
    # # Project nasc data
    # nasc.cps.proj <- st_as_sf(nasc.plot.cps, coords = c("long","lat"), crs = crs.geog)
    # nasc.cps.proj <- project_sf(nasc.cps.proj, crs.proj)
    # 
    # nasc.krill.proj <- st_as_sf(nasc.plot.krill, coords = c("long","lat"), crs = crs.geog)
    # nasc.krill.proj <- project_sf(nasc.krill.proj, crs.proj)
    
    # Map CPS backscatter and PWS dolphin group size
    pws.map <- base.map +
      # plot ship track data
      geom_sf(data = nav_sf, colour = "gray50", size = 0.5, alpha = 0.5) +
      # Plot NASC data
      geom_point(data = filter(nasc.plot.cps, NASC > 200), aes(X, Y, size = bin.level), fill = "blue", 
                 shape = 21, alpha = 0.75, show.legend = F) +
      # Plot PWS group size
      geom_point(data = pws.data.proj, aes(X, Y, size = bin.level), fill = "black", 
                 shape = 21, show.legend = F) +
      scale_size_area() +
      # Plot title
      ggtitle("CPS and Pacific whitesided dolphins") +
    ...
    # Save PWS plot
    ggsave(pws.map,
           filename = here("Figs/fig_pws_cps.png"),
    ...
    hb.map <- base.map +
      ...
      geom_sf(data = nav_sf, colour = "gray50", size = 0.5, alpha = 0.5) +
      # Plot NASC data
      geom_point(data = filter(nasc.plot.krill, NASC > 200), aes(X, Y, size = bin.level), fill = "pink", 
                 shape = 21, alpha = 0.75, show.legend = F) +
      # Plot PWS group size
      geom_point(data = hb.data.proj, aes(X, Y, size = bin.level), fill = "black", 
                 shape = 21, show.legend = F) +
      scale_size_area() +
      # Plot title
      ggtitle("Krill and Humpback whales") +
      coord_sf(crs = crs.proj, # CA Albers Equal Area Projection
               xlim = c(map.bounds["xmin"], map.bounds["xmax"]), 
               ylim = c(map.bounds["ymin"], map.bounds["ymax"]))
    ...
    # Save HB whale plot
    ggsave(hb.map,
           filename = here("Figs/fig_hbw_krill.png"),
           width = map.width, height = map.height) 
    
    # Combine mammal maps
    all.mammal.maps <- plot_grid(pws.map, hb.map, nrow = 1)
    
    ggsave(all.mammal.maps, 
           filename = here("Figs/fig_mammal_maps.png"),
...
wpts.sf <- wpts %>% 
  filter(type %in% c("Compulsory","Adaptive","Mammal","Saildrone")) %>% 
  st_as_sf(coords = c("long","lat"),crs = crs.geog)
...
haul.paths.sf <- st_as_sf(haul.paths, coords = c("long","lat"), crs = crs.geog) %>% 
  group_by(haul) %>% 
  summarise(do_union = F) %>% 
  st_cast("LINESTRING")

haul.locs.sf <- haul.mid %>% 
  rename(long = mid.long, 
         lat  = mid.lat) %>% 
...
cluster.catch.sf <- catch.pie %>%
  # left_join(select(haul.mid, Haul = haul, lat = mid.lat, long = mid.long)) %>% 
...
i.map <- leaflet() %>% 
  ...
  addLayersControl(
    overlayGroups = c("MPAs", "State Waters", "Exclusive Economic Zones", 
                      ...
                      "Saildrone Tracks", "Trawls",
                      "Backscatter-CPS", "Backscatter-Krill", "Backscatter (Large)",
                      "Backscatter-CPS (Small)","CUFES Egg Density", "CUFES (Negative)",
                      "Methane Seeps"),
    options = layersControlOptions(collapsed = F)) %>% 
  hideGroup(c("Backscatter (Large)", "Backscatter-Krill", "CUFES (Negative)",
              "Methane Seeps", "Saildrone Tracks","Backscatter-CPS (Small)")) %>% 
  fitBounds(imap.bounds$range.lon[1], imap.bounds$range.lat[1],
            imap.bounds$range.lon[2], imap.bounds$range.lat[2])

if (add.mammal.data) {
  i.map <- i.map %>% 
    # Add marine mammal observations
    addCircleMarkers(data = pws.data, radius = ~bin.level*2, color = "#000414", stroke = TRUE, weight = 1,
                     opacity = 0.75, fillOpacity = 0.75, fillColor =  "white",
                     popup = ~popup, label = ~label,
                     group =  "PWS dolphins") %>%
    addCircleMarkers(data = hb.data, radius = ~bin.level*2, color = "#000414", stroke = TRUE, weight = 1,
                     opacity = 0.75, fillOpacity = 0.75, fillColor =  "#276ddd",
                     popup = ~popup, label = ~label,
    ...
    addLayersControl(
      overlayGroups = c("MPAs", "State Waters", "Exclusive Economic Zones", 
                        ...
                        "Backscatter-CPS", "Backscatter-Krill", "Backscatter (Large)",
                        "Backscatter-CPS (Small)","CUFES Egg Density", "CUFES (Negative)",
                        "Methane Seeps","PWS dolphins", "Humpback whales"),
      options = layersControlOptions(collapsed = F)) %>% 
    hideGroup(c("Backscatter (Large)", "Backscatter-Krill", "CUFES (Negative)",
                "Methane Seeps", "Saildrone Tracks","Backscatter-CPS (Small)","PWS dolphins", "Humpback whales")) 
}
...
### Acoustic backscatter, CUFES Egg Density, and Trawl Species Proportions for the 2018 California Current Ecosystem Survey (1807RL) 

#### SWFSC Advanced Survey Technologies Group  

_**Last updated:**_ `r format(Sys.time(), format = "%F %T", tz = "GMT", usetz = T)`

An interactive map of the distribution of 38-kHz integrated backscattering coefficients (_s_~A~, m^2^ nmi^-2^;  averaged over 2000-m distance intervals) ascribed to CPS (from 5 to 70-m deep) and krill (5 to 350-m deep); CUFES egg density (eggs m^-3^) for anchovy, sardine, and jack mackerel; and proportions of CPS species in trawl clusters (white points) and some other miscellaneous surey data. Hover-over or click on map objects for more info.  
