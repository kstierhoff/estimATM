...
pacman::p_load(tidyverse,lubridate,here,sf,knitr,fs,leaflet,leaflet.extras,bookdown,
               leafem,mapview,htmltools,rnaturalearth,stplanr,ggrepel,plotly)
...
Files listed below are processed and plotted at any time, but ideally prior to being processed using plotSurvey, estimateBiomass, etc. 

```{r list-files, echo=TRUE}
# Controls
process.csv <- T # Process CSV files, or just re-generate HTML/plots
resize.map  <- F # Scale map to survey extent

# Platform name, used to name resulting image file
platform.name <- "RL"

if (process.csv) {
  # List full file paths
  csv.paths <- c(dir_ls(
    here::here("\\\\swc-storage4-s\\AST5\\SURVEYS\\20240625_LASKER_SummerCPS\\PROCESSED\\EV\\CSV\\"),
    regexp = "-Final 38 kHz CPS_nasc_cps.csv"))  
}

```

```{r other-paths, eval=FALSE, echo=FALSE}
# Paths
# 2024 Lasker
"\\\\swc-storage4-s\\AST5\\SURVEYS\\20240625_LASKER_SummerCPS\\PROCESSED\\EV\\CSV\\"
"-Final 38 kHz CPS_nasc_cps.csv"
# 2024 Long Beach Carnage
"\\\\swc-storage4-s\\AST5\\SURVEYS\\20240625_CARNAGE_SummerCPS\\PROCESSED\\EV\\CSV\\"
"-Final 38 kHz CPS_nasc_cps.csv"
# 2024 Lisa Marie
"\\\\swc-storage4-s\\AST5\\SURVEYS\\20240625_LISA-MARIE_SummerCPS\\PROCESSED\\EV\\CSV\\"
"-Final 38 kHz CPS_nasc_cps.csv"
# 2023 Lasker CCE
"C:\\SURVEY\\2307RL\\PROCESSED\\EV\\CSV\\LASKER"
"-Final 38 kHz CPS_nasc_cps.csv"
# 2023 Shimada hake survey
"\\\\swc-storage4-s\\AST4\\SURVEYS\\20230703_SHIMADA_SummerHake\\PROCESSED\\EV\\CSV"
"-Final 38 kHz CPS_nasc_cps.csv"
# 2023 Long Beach Carnage
"\\\\swc-storage4-s\\AST4\\SURVEYS\\20230708_CARNAGE_SummerCPS\\PROCESSED\\EV\\CSV"
"-Final 38 kHz CPS_nasc_cps.csv"
# 2023 Lisa Marie
"\\\\swc-storage4-s\\AST4\\SURVEYS\\20230703_LISA-MARIE_SummerCPS\\PROCESSED\\EV\\CSV"
"-Final 38 kHz CPS_nasc_cps.csv"
# 2023 Saildrone
"\\\\swc-storage4-s\\AST4\\SURVEYS\\20230703_SAILDRONE_SummerCPS\\PROCESSED\\EV\\CSV"
"-Final 38 kHz CPS_nasc_cps.csv"
```
...
write_csv(big.nasc, file = here("Output", paste0("nasc_big_plotCSV_", platform.name, ".csv")))
```
...
img.filename <- paste0("fig_backscatter_", platform.name, ".png")
