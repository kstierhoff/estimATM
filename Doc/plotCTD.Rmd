...
if (copy.files) {
  ...
  uctd.proc <- dir_ls(uctd.dir, regexp = uctd.hdr.pattern) %>% 
  ...
  ctd.proc <- dir_ls(ctd.dir, regexp = ctd.hdr.pattern) %>% 
    path_filter(regexp = "_processed")
...
```{r process-ctd}
# List raw UCTD ASCII files
ctd.hdr <- dir_ls(here("Data/CTD"), regexp = uctd.hdr.pattern) %>% 
    path_filter(regexp = "_processed", invert = TRUE)
...
if (length(ctd.hdr) == 0) {
  ...
  if (process.files) {
    ...
    ctd.proc <- dir_ls(here("Data/CTD"), regexp = uctd.hdr.pattern) %>% 
      path_filter(regexp = "_processed")
...
if (exists("uctd.sf")) {
  # Map casts
  mapview(nav_sf, zcol = "leg", legend = FALSE) +
    mapview(tx_sf, zcol = "Type", alpha = 0.5, layer.name = "Vessel track", legend = FALSE) +
    mapview(uctd.sf, zcol = "leg", cex = 4, layer.name = "UCTD Casts", legend = T)  
} else {
  include_graphics(here("Figs/fig_uctd_map.png"))
}
```  
...
if (exists("ctd.sf")) {
  # Map casts
  mapview(tx_sf, alpha = 0.5, layer.name = "Vessel track") +
    mapview(ctd.sf, zcol = c("leg"), cex = 4, layer.name = "CTD Casts", legend = T)  
} else {
  include_graphics(here("Figs/fig_ctd_map.png"))
}
```  
