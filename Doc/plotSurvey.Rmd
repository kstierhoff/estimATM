...
if (sampling.trawl) {
  if (get.db) {
    ...
    if (dbExistsTable(trawl.channel, "LengthFrequency"))
...
# Backscatter and trawl specifies proportions  

(ref:nasc-trawl) A map of the a) distribution of 38-kHz integrated backscattering coefficients ($s_A$, m^2^ nmi^-2^;  averaged over 2000-m distance intervals and from 5 to 70-m deep) ascribed to CPS, and b) proportions of CPS species (by weight) in trawl hauls (black points indicate trawl hauls with no CPS).

```{r plot-nasc-trawl-proportion-haul-wt,fig.cap='(ref:nasc-trawl)',out.width='100%'}
