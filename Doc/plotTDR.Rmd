...
process.all <- F # Process all TDR files
...
if(nrow(tbl.all.new) > 0) {
  for (k in unique(tbl.all.new$haul)) {
    ...
    tdr.plot <- filter(tbl.all.new, haul == k) %>% mutate(time = time + hours(tdr.offset[k]))
    ...
    tdr.plot <- tdr.plot %>% 
      filter(between(time,  
                     min(trawl.text$time) - minutes(10), 
                     max(trawl.text$time) + minutes(10)))
    ...
    plot.tdr <- ggplot() + 
      ...
      scale_y_continuous(limits = c(min(tdr.plot$depth),12), 
                         breaks = c(-50, -40, -30, -20, -10, seq(0, 12,4))) +
      labs(x = "Time (PST)",
           y = "Depth (m)",
  ...
  cat(paste0("Processed TDR data from haul(s) ", glue_collapse(unique(tbl.all.new$haul), sep = ",", last = " and "), "."))
...
Below are TDR data for the last three trawl hauls. **<span style="color: black;">Black</span>** lines are vessel speed over ground (SOG, knots), **<span style="color: red;">red</span>** is kite depth, and **<span style="color: blue;">blue</span>** is footrope depth. The dashed lines indicate the time between net deployment (or net in water) and retrieval (or net on deck), and the shaded **<span style="color: gray50;">gray</span>** region indicates the time during which the net was fishing (equilibrium until haul back).  
