...
CA, and $L_S$ ranged from `r length.summ$L.min[length.summ$scientificName == 'Engraulis mordax' & length.summ$stock == 'Central']` to `r length.summ$L.max[length.summ$scientificName == 'Engraulis mordax' & length.summ$stock == 'Central']` cm with modes at \~4, 6, and 9 cm. Although this is considered the best estimate of age-1+ biomass for the study area and period, the ATM may have underestimated young-of-the-year anchovy. To quantify the age-0 biomass in 2015, the 2016 estimate of abundance-at-length [@Stierhoff2020d] was projected back in time, in monthly steps, using a growth model, monthly records of commercial landings, and a published estimate of natural mortality (see **Section \@ref(discussion-anchovy-biomass-central)**).  
