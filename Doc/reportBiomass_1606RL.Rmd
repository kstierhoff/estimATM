...
(ref:survey-plan) Planned compulsory acoustic transect lines. Isobaths (light gray lines) are placed at 50, 200, 500, and 2,000 m (or approximately \~1,000 fathoms).
...
The `r tolower(survey.season)` `r survey.year` survey took place between Cape Scott, Vancouver Island and San Diego during `r survey.das` DAS between `r format(ymd(erddap.survey.start),"%d %B")` and `r format(ymd(erddap.survey.end),"%d %B %Y")`. Acoustic sampling was conducted by *`r survey.vessel`* along `r nrow(nasc.summ)` daytime east-west transects that totaled `r prettyNum(sum(nasc.summ$distance),big.mark=",",digits=0)` nmi. Catches from a total of `r nrow(haul)` nighttime surface trawls were combined into `r n_distinct(haul$cluster)` trawl clusters. As many as `r num2words(max(as.numeric(be$Stratum),na.rm = TRUE))` post-survey strata were defined considering transect spacing and the densities of echoes attributed to CPS. Biomasses and abundances were estimated for each species.
...
**Leg II**\
On 27 July, _`r survey.vessel`_ departed from MOC-P in Newport, OR at ~1430 and arrived at the start of transect 035 at ~1900 near Cascade Head to resume survey operations. On 31 July at ~2245, the EK-MUX froze; the survey resumed using only the EK60 until ~0600 on 1 August when it was repaired by replacing the IC chip. On 8 August, acoustic sampling ceased when a seawater cooling pipe broke, which required all echosounders to be secured to avoid hardware damage. On 8 August, _`r survey.vessel`_ arrived at Pier 32 in San Francisco, CA at ~0530 to complete Leg II. Repairs to the damaged pipe was scheduled to occur prior to Leg III. 

**Leg III**\
On 20 August, _`r survey.vessel`_ departed from Pier 32 in San Francisco at ~0130, and arrived at transect 071 near Point Cabrillo at ~1700 on 20 August to resume survey operations. The MS70 was still not functional from the end of the previous leg and remained offline for the remainder of the survey. The SX90 periodically froze and was rebooted throughout Leg III. Rough seas and high winds were encountered on 29 August; consequently, the ship slowed to 7 kn and began tacking on 30 August to minimize roll and preserve data quality. Only one trawl was successfully conducted on 29 August due to weather. UCTD operations were suspended for the remainder of Leg III. _`r survey.vessel`_ arrived at 10th Ave Marine Terminal in San Diego, CA on 2 September at ~1600 to complete Leg III.

**Leg IV**\
On 6 September, _`r survey.vessel`_ departed from 10th Ave Marine Terminal in San Diego, CA at ~2000, and arrived at transect 102 at ~1330 on 7 September to resume survey operations.  On 16 September, _`r survey.vessel`_ arrived at 10th Ave Marine Terminal in San Diego at ~1800 to complete the survey.  
...
The estimated biomass of the central stock of Northern Anchovy, found off CA south of Cape Mendocino, was `r prettyNum(sum(be.anch.c$Biomass), big.mark=",", digits = 5)` t (CI~95%~ = `r prettyNum(be$lower.ci.B[be$Species == "Engraulis mordax" & be$Stock == "Central" & be$Stratum == "All"],big.mark=",",digits=5)` - `r prettyNum(be$upper.ci.B[be$Species == "Engraulis mordax" & be$Stock == "Central" & be$Stratum == "All"],big.mark=",",digits=5)` t) in summer 2018, a nearly **five-fold increase from both the summer 2016 [151,558 t, CI~95%~ = 34,806 - 278,024; @Zwolinski2017] and summer 2017 estimates [153,460 t, CI~95%~ = 2,628 - 264,009 t; @Zwolinski2019a].** The length distribution of the stock in summer 2015 had two modes ($L_S$ **\~10 and 12 cm**), indicating the presence of two dominant year-classes.

### Pacific Sardine {#discussion-sardine}

#### Northern stock {#discussion-sardine-biomass-northern}

The summer 2015 survey sampled most of the potential habitat for the northern stock of Pacific Sardine, and likely most of the stock. The stock biomass was observed mostly offshore, by *`r survey.vessel`* **[EXPAND ON THIS THOUGHT?]**.

**A gap in the length distribution of Pacific Sardine between 15 and 18 cm indicates poor recruitment in 2015. Accordingly, the stock abundance and biomass declined between 2016 and 2017, and the modal length increased from 17-19 to 21-23 cm. Similar to 2017, few trawls with Pacific Sardine smaller than 10 cm indicates that recruitment was weak again in 2018.**
