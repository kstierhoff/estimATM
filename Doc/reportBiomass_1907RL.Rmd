...
(ref:nasc-ns-all) Spatial distributions of 38-kHz integrated backscattering coefficients ($s_A$, m^2^ nmi^-2^;  averaged over 2000-m distance intervals and from 5 to 70 m deep) ascribed to CPS from nearshore sampling using a) F/V _Lisa Marie_, b) a USV, and c) F/V _Long Beach Carnage_. **[Consider replacing or removing this figure; however, it's currently included because nearshrore backscatter is not visible in Fig. \@ref(fig:nasc-cufes-trawl)a]**

```{r nasc-ns-all, fig.cap='(ref:nasc-ns-all)',out.width='7.75in',fig.pos='H'}
include_graphics(here("Figs/fig_backscatter_cps_AllNearshore.png"))
```
