...
This document, and references herein, describes in detail the ATM as presently used by NOAA's Southwest Fisheries Science Center (SWFSC) to survey the distributions and abundances of CPS and their oceanographic environments [e.g., @Cutter2008;@Demer2012;@Zwolinski2014]. In general terms, the contemporary ATM combines information from satellite-sensed oceanographic conditions, calibrated multifrequency echosounders, probe-sampled oceanographic conditions, pumped samples of fish eggs, and trawl-net catches of juvenile and adult CPS. The survey area is initially defined with consideration to the expected distribution of a priority stock or stock assemblage, in this case, the central stock of Northern Anchovy (**Fig. \@ref(fig:anchovy-distribution)**). The survey area is further expanded to encompass as much of the potential habitat as possible for other CPS present off the West Coast of the U.S., as time permits.  
...
Additional sampling was conducted nearshore along 5-nmi-long transects spaced 5 nmi apart between Pt. Conception and San Diego using F/V _Long Beach Carnage_ equipped with multi-frequency (**MTA4**) echosounders (magenta lines, **Fig. \@ref(fig:survey-plan)**). The goal of the nearshore sampling was to estimate the abundance and biomass of the central stock of Northern Anchovy and northern stock of Pacific Sardine close to shore, in shallow water, or both, where _`r survey.vessel`_ could not safely navigate or trawl.  
...
(ref:survey-plan) Planned compulsory (solid black lines) transect lines sampled by _`r survey.vessel`_ and nearshore transect lines sampled by F/V _Long Beach Carnage_ (solid magenta lines). Isobaths (light gray lines) are placed at 50, 200, 500, and 2,000 m (or approximately 25, 100, 250, and 1,000 fathoms). _`r survey.vessel`_'s actual daytime and nighttime movements are overlaid (heavy gray line). **[Kevin will revise to add LBC's path, if possible. It's not presently there.]**
...
In addition to echoes from target CPS, echoes may also be present from other CPS (Pacific Saury, _Cololabis saira_), or semi-demersal fish such as Pacific Hake and rockfishes ($Sebastes$ spp.). When analyzing the acoustic-survey data, it was therefore necessary to manually filter “acoustic by-catch,” i.e., backscatter not from the target species. Fish echoes in proximity to rocky bottoms and vaguely diffuse backscatter were excluded from the CPS analysis using a custom interactive script for R (**Fig. \@ref(fig:ctd-app)**).  
...
(ref:ctd-app) Reconstructed echograms in R: Integrated echo energy versus depth for all “CPS-like” targets and (top); and integrated echo energy versus depth excluding likely rockfishes (_Sebastes_ spp.) and near-surface reverberation. The black line represents the bottom and the red and blue lines represent the final upper and lower integration depths by interval, respectively.
...
```{r trawl-cluster-map,fig.cap='(ref:trawl-cluster-map)',out.width='8.5in',fig.pos='H'}
...
Despite most transects having ended with practically zero biomass densities, and a subsequent survey showing virtually no anchovy spawning outside the area surveyed by _`r survey.vessel`_ (**Fig. \@ref(fig:offshore-anchovy)**), the decline in biomass along _`r survey.vessel`_ transects is sharp, raising the question whether or not _`r survey.vessel`_ reached the absolute western edge of the anchovy distribution. Information collected during the subsequent summer survey will provide quantitative information on the possible bias.  

### Pacific Sardine {#discussion-sardine}     
#### Northern stock {#discussion-sardine-biomass-northern}  

The potential habitat of the northern stock of Pacific Sardine was widespread in the core survey area (**Fig. \@ref(fig:sardine-potential-habitat)a**). However, the majority of Pacific Sardine were encountered nearshore, in waters where the model indicated unsuitable habitat due to excessive chlorophyll a concentration (**Fig. \@ref(fig:sardine-potential-habitat)c**). Nonetheless, the temperatures associated to those sardine were in the range of those associated to the northern stock (**Fig. \@ref(fig:sardine-potential-habitat)b**). Therefore, the the `r prettyNum(sum(be.sar.s$Biomass), big.mark=",", digits = 5)` t (CI~95%~ = `r prettyNum(be$lower.ci.B[be$Species == "Sardinops sagax" & be$Stock == "Northern" & be$Stratum == "All"], big.mark=",",digits=5)` - `r prettyNum(be$upper.ci.B[be$Species == "Sardinops sagax" & be$Stock == "Northern" & be$Stratum == "All"],big.mark=",",digits=5)` t) of biomass estimated there were attributed to the northern stock of Pacific Sardine.

### Pacific Mackerel {#discussion-mack}  

**[Juan will update the remaining Discussion sections for Pac. and Jack Mackerel in the next version.]**
...
**[Juan will update the biomass time series section in the next version.]**

**[Kevin will update the biomass time series plots soon; needs to rebuild the time series table in the AST database]**
...
The authors greatly appreciate that the ATM surveys require an enormous effort by multiple groups of people, particularly the Advanced Survey Technologies group (Gabriel Johnson and Scott Mau) and biological sampling team (N. Bowlin, B. Erisman, M. Human, B. Schwartzkopf, A. Thompson, A. Freire, L. Giuseffi, V. Hermanson, K. James, O. Snodgrass, W. Watson); the officers and crew of _Lasker_; and the Fisheries Resources Division administrative staff. We thank Capts. Rich Ashley and Tom Brinton (F/V _Long Beach Carnage_) for their coordination and cooperation during the nearshore sampling, and to Joel van Noord for leading the at-sea processing of purse seine specimens. We thank Diane Pleschner-Steele for contracting the _Long Beach Carnage_ to conduct the nearshore survey of the SCB. We thank Dianna Porzio, Trung Nguyen, Kelly Kloos, and Trevor Stocking (CA Department of Fish and Wildlife) for their assistance processing specimens from purse-seine set from the Long Beach Carnage. Finally, reviews by **[List of reviewers]** improved this report.  
...
(ref:offshore-anchovy) Distribution of northern anchovy inferred from the 2021 spring ATM and CalCOFI surveys. From the ATM survey, the red circles represent the lowest biomass densities that collectively amount to 5% of the total biomass, and the green circles represent the largest biomass densities that collectively amount to 95% of the biomass. From the CalCOFI survey, which occurred less than one months after the ATM survey, the blue circles indicate the lowest egg densities collected by CUFES amounting to 5% of all the anchovy eggs, and the “+” symbols indicate the CUFES samples that amount to 95% of all the eggs.
