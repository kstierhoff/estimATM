...
The numerical densities by length class (**Section \@ref(methods-trawl-clustering)**) were averaged for each stratum in a similar way as that used for biomass in Equation \@ref(eq:biomass-mean-density), and multiplied by the stratum area to obtain abundance per length class.  
...
(ref:sardine-stock-allocation) Relative abundance versus standard length ($L_S$) for Pacific Sardine (_Sardinops sagax_) estimated during the spring 2021 (top) and the summer 2021 survey [@Stierhoff2023a], separated by putative southern (middle) and northern stocks (bottom).
...
There was a negligible amount of Pacific mackerel observed by _`r survey.vessel`_ and _Long Beach Carnage_. See the summer 2021 CCE biomass report [@Stierhoff2023a] for a better indication of the status of this stock.  

### Jack Mackerel {#discussion-jack}  

The estimated biomass of Jack Mackerel in the core survey region was `r prettyNum(be.all$Biomass[be.all$Species == 'Trachurus symmetricus'],big.mark=",",digits=3)` t (CI~95%~ = `r prettyNum(be.all$lower.ci.B[be.all$Species == 'Trachurus symmetricus'],big.mark=",",digits=3)` - `r prettyNum(be.all$upper.ci.B[be.all$Species == 'Trachurus symmetricus'],big.mark=",",digits=3)` t), which is less than 5% of the biomass estimated in the summer 2019 survey. See the summer 2021 CCE biomass report [@Stierhoff2023a] for a better indication of the status of this stock.  
