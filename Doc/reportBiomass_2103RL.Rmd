...
(ref:trawl-cluster-map) Polygons enclosing 100-m acoustic intervals from _`r survey.vessel`_ assigned to each trawl cluster (a), and the acoustic proportions of CPS in trawl clusters (b). The numbers inside each polygon (a) are the cluster numbers, which are located at the center of mass of all trawls in that cluster. Trawl clusters with no CPS present are indicated as black points (b).
...
The `r tolower(survey.season)` `r survey.year` survey, between San Francisco and San Diego, was `r survey.das` DAS between `r format(ymd(erddap.survey.start),"%d %B")` and `r format(ymd(erddap.survey.end),"%d %B %Y")`. In the core area, acoustic sampling was conducted along `r nrow(nasc.summ)` daytime east-west transects that totaled `r prettyNum(sum(nasc.summ$distance),big.mark=",",digits=2)` nmi. Catches from a total of `r nrow(haul)` nighttime surface trawls were combined into `r n_distinct(haul$cluster)` trawl clusters. As many as `r num2words(max(as.numeric(be$Stratum),na.rm = TRUE))` post-survey strata were defined considering transect spacing and the densities of echoes attributed to CPS. In the nearshore survey area, acoustic sampling was conducted along `r nrow(nasc.summ.ns)` daytime east-west transects by _Long Beach Carnage_ totaling `r prettyNum(sum(nasc.summ.ns$distance),big.mark=",",digits=2)` nmi. As many as `r num2words(max(as.numeric(be.ns$Stratum),na.rm = TRUE))` post-survey strata were defined considering transect spacing and the densities of echoes attributed to CPS. Biomasses and abundances were estimated for each species in the core and nearshore survey areas.  

## Acoustic backscatter {#results-backscatter-distribution} 

Acoustic backscatter ascribed to CPS was observed throughout the core survey area, but was most prevalent south of Monterey Bay, especially offshore in the Southern CA Bight south of the northern Channel Islands (**Fig. \@ref(fig:nasc-cufes-trawl)a**). Multiple transects were extended (**Appendix \@ref(appendix-anchovy-distribution)**) to ensure zero-biomass intervals at their offshore ends. Acoustic backscatter ascribed to CPS was also observed throughout the nearshore survey area, but was most prevalent between Long Beach and Oceanside, CA (**Fig. \@ref(fig:nasc-cufes-trawl)a**). The majority (greater than `r cum.biomass.limit*100`%) of biomass for each species was apportioned using catch data from trawl clusters conducted within a distance shorter than 20 nmi (**Fig. \@ref(fig:biomass-cluster-distance)**).  

## Egg densities and distributions {#results-cufes-density}  

Northern Anchovy eggs were abundant in CUFES samples throughout the survey area south of Monterey Bay, but were most abundant and coincident with much of the observed acoustic backscatter in the offshore portion of transects in the Southern CA Bight (**Fig. \@ref(fig:nasc-cufes-trawl)b**). A few samples in the Southern CA Bight contained Pacific Sardine eggs, mostly along the southeast coast of Santa Catalina Island (**Fig. \@ref(fig:nasc-cufes-trawl)b**). No Jack Mackerel eggs were observed.  

## Trawl catch {#results-trawl-catch}  

Northern Anchovy was the predominant species in trawl catches throughout the survey area (**Fig. \@ref(fig:nasc-cufes-trawl)c**). Relative to Northern Anchovy, small numbers of Pacific Sardine were collected offshore of the northern Channel Islands and east of San Nicolas Island (**Fig. \@ref(fig:nasc-cufes-trawl)c**; not visible at this scale). Jack Mackerel were present in several trawls offshore in the Southern CA Bight, and one near Long Beach, and small numbers of Pacific Mackerel were caught west of San Nicolas Island (**Fig. \@ref(fig:nasc-cufes-trawl)c**; not visible at this scale). Overall, the `r nrow(haul)` trawls captured a combined `r prettyNum(sum(cluster.summ.wt$AllCPS),big.mark=",",digits=3)` kg of CPS (`r prettyNum(sum(cluster.summ.wt$'Anchovy'),big.mark=",",digits=3)` kg of Northern Anchovy, `r prettyNum(sum(cluster.summ.wt$'Sardine'),big.mark=",",digits=3)` kg of Pacific Sardine, `r prettyNum(sum(cluster.summ.wt$'PacMack'),big.mark=",",digits=3)` kg of Pacific Mackerel, and `r prettyNum(sum(cluster.summ.wt$'JackMack'),big.mark=",",digits=3)` kg of Jack Mackerel; no Pacific Herring were collected).  
...
#   bind_rows(be.table.sub.ns) %>% 
#   ungroup() %>% 
#   select(Name, Stock, Region, everything(), -Stock)
...
<!-- In the nearshore region, the relatively small amount of biomass was `r prettyNum(be.all.ns$Biomass[be.all.ns$Species == 'Trachurus symmetricus'],big.mark=",",digits=3)` t (CI~95%~ = `r prettyNum(be.all.ns$lower.ci.B[be.all.ns$Species == 'Trachurus symmetricus'],big.mark=",",digits=3)` - `r prettyNum(be.all.ns$upper.ci.B[be.all.ns$Species == 'Trachurus symmetricus'],big.mark=",",digits=3)` t, CV = `r prettyNum(be.all.ns$biomass.cv[be.all.ns$Species == 'Trachurus symmetricus'],big.mark=",",digits=1)`%; **Table \@ref(tab:biomass-jack)**, **Fig. \@ref(fig:biom-dens-jack)b**) **[Check this after receipt of purse seine catch from CDFW]**; was distributed around Point Conception, Los Angeles, and Santa Catalina Island (**Fig. \@ref(fig:l-disagg-jack)**); and had a length distribution similar to the core region (**Table \@ref(tab:l-freq-summ-jack)**). Biomass in the nearshore region comprised `r prettyNum((be.all.ns$Biomass[be.all.ns$Species == 'Trachurus symmetricus'] / (be.all.ns$Biomass[be.all.ns$Species == 'Trachurus symmetricus'] + be.all$Biomass[be.all$Species == 'Trachurus symmetricus']))*100,big.mark=",",digits=2)`% of the total.   -->
...
The principal objective of the `r survey.das`-day, Spring 2021 CPS survey was to assess the central stock of Northern Anchovy in U.S. waters. Then, as possible, estimates were also sought for Pacific Sardine, Pacific Mackerel, and Jack Mackerel in the survey area. With the benefit of favorable weather and few technical problems, _`r survey.vessel`_ surveyed from San Diego to San Francisco. All transects south of Point Conception, where the biomass of Northern Anchovy was expected to be greatest, were spaced 15-nmi apart to reduce the variance; north of Point Conception, transects were spaced 20-nmi apart to maximize the extent of the survey area with the available days at sea.  

## Biomass and abundance of CPS
### Northern Anchovy {#discussion-anchovy-biomass} 
#### Central stock {#discussion-anchovy-biomass-central}  
\
The estimated biomass of the central stock of Northern Anchovy in the core survey region was `r prettyNum(sum(be.anch.c$Biomass), big.mark=",", digits = 5)` t (CI~95%~ = `r prettyNum(be$lower.ci.B[be$Species == "Engraulis mordax" & be$Stock == "Central" & be$Stratum == "All"],big.mark=",",digits=5)` - `r prettyNum(be$upper.ci.B[be$Species == "Engraulis mordax" & be$Stock == "Central" & be$Stratum == "All"],big.mark=",",digits=5)` t) in spring 2021, which represents a `r round(1/(sum(be.anch.c$Biomass)/810634)*100)`% increase over the biomass (810,634 t; CI~95%~ = 587,317 - 1,066,265) observed in summer 2019 [@Stierhoff2020b]. The length distribution of the stock in spring 2021 had one mode ($L_S$ between ~10 and 13 cm), likely corresponding to an abundant cohort from 2020.   

Despite most _`r survey.vessel`_ transects having ended with practically zero biomass densities at their western ends, the declines are sharp, raising the question of whether or not _`r survey.vessel`_ reached the absolute western edge of the anchovy distribution in the Southern CA Bight. Analysis of egg distribution data from the spring CalCOFI survey indicates that the abundance of anchovy offshore of the survey area is likely small (**Appendix \@ref(appendix-anchovy-distribution)**).  Data collected during the subsequent ATM survey in Summer 2021 provides more information on this possible bias.  

### Pacific Sardine {#discussion-sardine}     
#### Southern stock {#discussion-sardine-southern}  
\
The core survey area included some "good" and "optimal" habitat for the northern stock of Pacific Sardine, particularly north of Point Conception (**Fig. \@ref(fig:sardine-potential-habitat)a**). However, the Pacific Sardine sampled in the Southern CA Bight were exclusively in the nearshore regions and around the northern Channel Islands, which had unsuitable habitat due to excessive chlorophyll-a concentration (**Fig. \@ref(fig:sardine-potential-habitat)b**). Therefore, the Pacific Sardine observed in the spring 2021 survey, with a biomass of `r prettyNum(be.all.summ$Biomass[be.all.summ$Species == "Sardinops sagax" & be.all.summ$Stock == "Southern"], big.mark=",",digits=5)` t (CI~95%~ = `r prettyNum(be.all.summ$lower.ci.B[be.all.summ$Species == "Sardinops sagax" & be.all.summ$Stock == "Southern"], big.mark=",",digits=4)` - `r prettyNum(be.all.summ$upper.ci.B[be.all.summ$Species == "Sardinops sagax" & be.all.summ$Stock == "Southern"],big.mark=",",digits=5)` t), were attributed to the southern stock. Pacific Sardine eggs were absent from the nearshore, suggesting that the Pacific Sardine, with a modal length between 12 and 13 cm, were immature, age-0 fish that apparently recruited locally to the southern stock because their forecast growth matched the 16 to 18 cm lengths of Pacific Sardine sampled in the Southern CA Bight in summer survey [@Stierhoff2022a]. Furthermore, few or none of them migrated and recruited to the northern stock because there were no Pacific Sardine with comparable sizes in the areas sampled off Oregon and Washington (in northern stock habitat) during summer 2021 (**Fig. \@ref(fig:sardine-stock-allocation)**).  
...
There was a negligible amount of Pacific mackerel observed by _`r survey.vessel`_ and _Long Beach Carnage_. The subsequent ATM survey in summer 2021 will provide a better indication of the status of this stock.  

### Jack Mackerel {#discussion-jack}  

The estimated biomass of Jack Mackerel in the core survey region was `r prettyNum(be.all$Biomass[be.all$Species == 'Trachurus symmetricus'],big.mark=",",digits=3)` t (CI~95%~ = `r prettyNum(be.all$lower.ci.B[be.all$Species == 'Trachurus symmetricus'],big.mark=",",digits=3)` - `r prettyNum(be.all$upper.ci.B[be.all$Species == 'Trachurus symmetricus'],big.mark=",",digits=3)` t), which is less than 5% of the biomass estimated in the summer 2019 survey. The subsequent ATM survey in summer 2021 will provide a better indication of the status of this stock.  
...
# Investigation of un-sampled Northern Anchovy offshore of the surveyed area {#appendix-anchovy-distribution}

The biomass of the central stock of Northern Anchovy occupied mostly the median and western sections of the survey (**Fig. \@ref(fig:biom-dens-anch-c)a**), which raised the question of whether or not _`r survey.vessel`_ reached the absolute edge of the anchovy distribution. There are two indicators that reassures us that if anchovy were to the west of the survey area, there might have been in low abundances. The first indicator is the fact that all transects ended with practically zero biomass on their western ends (**Fig. \@ref(fig:offshore-anchovy)**); The second is the lack of anchovy eggs west of the _`r survey.vessel`_’s footprint during the subsequent CalCOFI survey, just less than a month later than the ATM survey. While the above cannot prove that there wasn’t a substantial amount of anchovy outside, at least it hints at the opposite.  

(ref:offshore-anchovy) Distribution of Northern Anchovy inferred from the 2021 spring ATM and CalCOFI surveys. From the ATM survey, the red circles represent the lowest biomass densities that collectively amount to 5% of the total biomass, and the green circles represent the largest biomass densities that collectively amount to 95% of the biomass. From the CalCOFI survey, which occurred less than one month after the ATM survey, the blue circles indicate the lowest egg densities collected by CUFES amounting to 5% of all the Northern Anchovy eggs, and the “+” symbols indicate the CUFES samples that amount to 95% of all the eggs.
