...
**[Remove redundant columns in tables for all species/stocks]**
...
The estimated biomass of the southern stock of Pacific Sardine in the survey region was `r prettyNum(be.all.summ$Biomass[be.all.summ$Species == "Sardinops sagax" & be.all$Stock == "Southern"], big.mark=",", digits = 5)` t (CI~95%~ = `r prettyNum(be.all.summ$lower.ci.B[be.all.summ$Species == "Sardinops sagax" & be.all.summ$Stock == "Southern"], big.mark=",",digits=5)` - `r prettyNum(be.all.summ$upper.ci.B[be.all.summ$Species == "Sardinops sagax" & be.all.summ$Stock == "Southern"],big.mark=",",digits=5)` t). In summer 2021, `r prettyNum(sum(be.sar.s$Biomass[be.sar.s$Stratum %in% c(2:4)]), big.mark=",", digits = 5)` t (strata 2-4; `r prettyNum(sum(be.sar.s$Biomass[be.sar.s$Stratum %in% c(2:4)])/sum(be.sar.s$Biomass)*100, big.mark=",", digits = 1)`% of the total biomass) of southern stock biomass was observed in U.S. waters, and the remaining `r prettyNum(sum(be.sar.s$Biomass[be.sar.s$Stratum ==1]), big.mark=",", digits = 5)` t (stratum 1; `r prettyNum(sum(be.sar.s$Biomass[be.sar.s$Stratum ==1])/sum(be.sar.s$Biomass)*100, big.mark=",", digits = 1)`% of the total biomass) was observed off Baja CA. In summer 2019, the estimated biomass of the southern stock of Pacific Sardine in U.S. waters was 8,322 t [@Stierhoff2020b]. The summer 2021 survey area was the first to survey south of the U.S.-Mexico border.  **[Describe, tabulate, or plot the time series of southern stock sardine in U.S. waters. State when the southern stock was first observed in the ATM surveys and if there is an increasing trend.]**  

### Pacific Mackerel {#discussion-mack}  

In summer 2021, the estimated biomass of Pacific Mackerel in the survey region was `r prettyNum(be.all.summ$Biomass[be.all.summ$Species == 'Scomber japonicus'],big.mark=",",digits=3)` t (CI~95%~ = `r prettyNum(be.all.summ$lower.ci.B[be.all.summ$Species == 'Scomber japonicus'],big.mark=",",digits=3)` - `r prettyNum(be.all.summ$upper.ci.B[be.all.summ$Species == 'Scomber japonicus'],big.mark=",",digits=3)` t), which is not significantly different from the 24,643 t estimated in summer 2019, and is within the range of 8,000 t [summer 2013; @Zwolinski2014] to 41,139 t [summer 2018; @Zwolinski2019a] estimated during summer surveys since 2013 (**Figs. \@ref(fig:biomass-ts-line)** and **\@ref(fig:biomass-ts-bar)**).  

### Jack Mackerel {#discussion-jack}  

In summer 2021, the estimated biomass of Jack Mackerel in the survey region, south of Cape Flattery, was `r prettyNum(be.all.summ$Biomass[be.all.summ$Species == 'Trachurus symmetricus'],big.mark=",",digits=3)` t (CI~95%~ = `r prettyNum(be.all.summ$lower.ci.B[be.all.summ$Species == 'Trachurus symmetricus'],big.mark=",",digits=3)` - `r prettyNum(be.all.summ$upper.ci.B[be.all.summ$Species == 'Trachurus symmetricus'],big.mark=",",digits=3)` t), which is `r round(be.all.summ$Biomass[be.all.summ$Species == 'Trachurus symmetricus']/385801,1)`-fold higher than 385,801 t estimated in summer 2019 [@Stierhoff2020b]. In summer 2021, Jack Mackerel was the second most abundant CPS overall, and comprised `r round(be.all.summ$Biomass[be.all.summ$Species == "Trachurus symmetricus"]/sum(be.all.summ$Biomass)*100)`% of the total CPS biomass (**Figs. \@ref(fig:biomass-ts-line)** and **\@ref(fig:biomass-ts-bar)**).  

### Pacific Herring {#discussion-herring} 

In summer 2021, the estimated biomass of Pacific Herring in the survey region, south of Cape Flattery, was `r prettyNum(be.all.summ$Biomass[be.all.summ$Species == 'Clupea pallasii'],big.mark=",",digits=3)` t (CI~95%~ = `r prettyNum(be.all.summ$lower.ci.B[be.all.summ$Species == 'Clupea pallasii'],big.mark=",",digits=3)` - `r prettyNum(be.all.summ$upper.ci.B[be.all.summ$Species == 'Clupea pallasii'],big.mark=",",digits=3)` t). This was `r round(be.all.summ$Biomass[be.all.summ$Species == 'Clupea pallasii']/267792*100,1)`% of the 267,792 t estimated in summer 2019 [@Stierhoff2020b], when the survey also included the west coast of Vancouver Island, Canada [@Stierhoff2020b]. In summers 2021, 2018 [@Stierhoff2019b], and 2017 [@Zwolinski2019a], the stock was closer to shore and more patchily distributed compared to summer 2019 when the stock spanned a greater portion of the continental shelf [@Stierhoff2020b]. **[For what survey domains? Are they the same during 2019 and 2021?]**  

### Round Herring {#discussion-rherring} 

In summer 2021, the estimated biomass of Round Herring in the core survey region was `r prettyNum(be.all.summ$Biomass[be.all.summ$Species == 'Etrumeus acuminatus'],big.mark=",",digits=3)` t (CI~95%~ = `r prettyNum(be.all.summ$lower.ci.B[be.all.summ$Species == 'Etrumeus acuminatus'],big.mark=",",digits=3)` - `r prettyNum(be.all.summ$upper.ci.B[be.all.summ$Species == 'Etrumeus acuminatus'],big.mark=",",digits=3)` t), and was observed entirely in the southernmost portion **[Where? Give two landmarks bounding the distribution.]** of the survey area surveyed by _`r survey.vessel`_ and _Carranza_. This was the first time the SWFSC's ATM survey has encountered Round Herring and estimated its biomass.  
...
**[KLS] This text has been present in the last several biomass reports. Revise to reflect latest results, particularly the general increase in biomass for the forage fish community, but particularly the resurgence and dominance of anchovy. Perhaps note that the NS of sardine remains low, and that the SS biomass is a significant part of the CPS community in the Southern CA Bight.]**

**[DAD] Consider using different shades of red and green for each stock, and adding both stocks of sardine and anchovy to Figs 33 and 34. In the text, describe the different sampling areas for each survey, or limit the data to a common area, e.g. from Cape Flattery to San Diego(?). Thoughts?**

**[KLS] Added SS sardine and NS anchovy to biomass time series plots. Please comment on color choices.**
