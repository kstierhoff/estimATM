...
author: 
  ...
  - I.R. Rojas-Gonz\'alez, Instituto Nacional de Pesca y Acuacultura (INAPESCA)
  - J.R.F. Vallarta-Z\'arate, Instituto Nacional de Pesca y Acuacultura (INAPESCA)
...
Total estimated biomass of eight stocks (six species) within the survey area was `r prettyNum(sum(be.all.summ$Biomass), big.mark=",", digits = 5)` t. Of this `r round(be.all.summ$Biomass[be.all$Species == "Engraulis mordax" & be.all$Stock == "Central"]/sum(be.all.summ$Biomass)*100)`% (`r prettyNum(be.all.summ$Biomass[be.all.summ$Species == "Engraulis mordax" & be.all$Stock == "Central"], big.mark=",", digits = 5)` t) was attributed to the central stock of Northern Anchovy and `r round(be.all.summ$Biomass[be.all$Species == "Sardinops sagax" & be.all$Stock == "Southern"]/sum(be.all.summ$Biomass)*100)`% (`r prettyNum(be.all.summ$Biomass[be.all.summ$Species == "Sardinops sagax" & be.all$Stock == "Southern"], big.mark=",", digits = 5)` t) to the southern stock of Pacific Sardine, both of which were surveyed in their entirety. Contributions by other stocks were northern stock of Northern Anchovy (`r round(be.all.summ$Biomass[be.all$Species == "Engraulis mordax" & be.all$Stock == "Northern"]/sum(be.all.summ$Biomass)*100, 1)`%), northern stock of Pacific Sardine (`r round(be.all.summ$Biomass[be.all$Species == "Sardinops sagax" & be.all$Stock == "Northern"]/sum(be.all.summ$Biomass)*100, 1)`%), Pacific Mackerel (`r round(be.all.summ$Biomass[be.all$Species == "Scomber japonicus" & be.all$Stock == "All"]/sum(be.all.summ$Biomass)*100, 1)`%), Jack Mackerel (`r round(be.all.summ$Biomass[be.all$Species == "Trachurus symmetricus" & be.all$Stock == "All"]/sum(be.all.summ$Biomass)*100)`%), Pacific Herring (`r round(be.all.summ$Biomass[be.all$Species == "Clupea pallasii" & be.all$Stock == "All"]/sum(be.all.summ$Biomass)*100, 1)`%), and Round Herring (`r round(be.all.summ$Biomass[be.all$Species == "Etrumeus acuminatus" & be.all$Stock == "All"]/sum(be.all.summ$Biomass)*100, 1)`%).
...
This survey was conducted with the approval of the Secretaria de Relaciones Exteriores (SRE, Diplomatic note CTC/1312/2021), the Instituto Nacional de Estadística y Geografía (INEGI; Permit: EG0082021), and the Comisión Nacional de Acuacultura y Pesca (CONAPESCA; Permit: PPFE/DGOPA-073/21). **[List all SRPs?]**  
...
To estimate the abundances and biomasses of CPS close to shore, in shallow water, or both, where _`r survey.vessel`_ could not efficiently or safely navigate or trawl, acoustic and purse seine sampling was conducted nearshore between Cape Flattery and San Diego using two fishing vessels equipped with multi-frequency echosounders (magenta lines, **Fig. \@ref(fig:survey-plan)**). _Lisa Marie_ sampled 5-nmi-long transects spaced 5 nmi apart between Cape Flattery and Bodega Bay. _Long Beach Carnage_ sampled 5-nmi-long transects spaced 5 nmi apart between Bodega Bay and San Diego, and 2.5-nmi-long transects spaced 2.5 nmi apart around Santa Cruz and Santa Catalina Islands in the Southern CA Bight.  

(ref:sardine-potential-habitat) Distribution of potential habitat for the northern stock of Pacific Sardine, temporally aggregated using an average of the habitat centered ±2° around the positions of _`r survey.vessel`_ (black lines), _Carranza_ (green lines), and USVs (cyan lines; **[What happened with the southern-most Saildrone transect off OR?]**) throughout the survey. Areas in white correspond to no available data, e.g., cloud coverage preventing satellite-sensed observations **[Replot with habitat data extending to the southern extent of the survey, when available - DAD]**.
...
(ref:survey-plan) Planned compulsory and adaptive transects sampled by _`r survey.vessel`_ and _Carranza_; interstitial and offshore transects sampled by USVs; and nearshore transects sampled by _Lisa Marie_ and _Long Beach Carnage_. Isobaths (light gray lines) are 50, 200, 500, and 2,000 m (or approximately 27, 109, 273, and 1,094 ftm, respectively) **[Consider reordering Figures 2 and 3 such that the planned transect lines are presented before the actual transect lines. The text may need to be finessed to present the habitat after the transect plan, which is theoretically designed with consideration to the habitat.]**.
...
(ref:tdr-example) An example plot showing the depth (m) of the trawl headrope (red line) and footrope (blue line) measured using temperature-depth recorders (TDRs) during the time that the net was deployed (dashed box) and actively fishing (shaded region). The vessel speed over ground (kn, magenta line) was measured using the ship's GPS. **[Zoom speed axis to 0-12 kn, add 2 kn gridlines and 4 kn labels. Move “SOG (kn)” to y-axis label. Add 5 min gridlines and 10 min labels to time axis. Change time-stamps to “Net in water”, “Equilibrium”, “Haul Back”, and “Net on deck”.]**
...
Up to three trawls were conducted each night as described in **Section \@ref(methods-trawl-locations-lasker)**, for _`r survey.vessel`_.  
...
### Biomass and sampling precision estimates {#methods-biomass-and-precision}  
...
### Percent biomass per cluster {#methods-cluster-biomass-density}  
...
(ref:l-freq-summ-jack) Abundance versus fork length ($L_F$, cm) for Jack Mackerel (_Trachurus symmetricus_) in the core and nearshore survey regions. **[Split columns to fit on one page, if possible; current effort negatively affects page layout for other figures/tables.]**
...
be.table.sub <- be.table %>% 
  ...
  bind_rows(be.table.sub.ns) %>% 
  ungroup() %>% 
  select(everything(), -Name, -Stock, -Region)
...
This is the first time that the SWFSC's ATM sampling was coordinated with INAPESCA's sampling from _Carranza_. This historic collaboration facilitated the sampling of multiple stocks that span the U.S.-Mexico border, and resulted in estimations of the biomasses and distributions of the entire central stock of Northern Anchovy and the entire southern stock of Pacific Sardine.  

**[Describe the unique aspects of each survey that may impact the estimates or species/stocks encountered. In 2021, for example, we surveyed well into Mexico which had considerable effects on the total biomass. I may have mentioned that briefly in one of the species subsections of the Discussion. In 2022, that may require 2 or more paragraphs to describe the impacts of lost time and different vessels conducting large portions of the survey with unproven methods.]**  
