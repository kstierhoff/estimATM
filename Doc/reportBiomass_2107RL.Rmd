...
<!-- Since 2018, the biomass of the southern stock of Pacific Sardine in U.S. waters has increased from 33,093 to `r prettyNum(be.all.summ$Biomass[be.all.summ$Species == "Sardinops sagax" & be.all$Stock == "Southern"], big.mark=",", digits = 5)` t (CI~95%~ = `r prettyNum(be.all.summ$lower.ci.B[be.all.summ$Species == "Sardinops sagax" & be.all.summ$Stock == "Southern"], big.mark=",",digits=5)` - `r prettyNum(be.all.summ$upper.ci.B[be.all.summ$Species == "Sardinops sagax" & be.all.summ$Stock == "Southern"],big.mark=",",digits=5)` t), with `r prettyNum(sum(be.sar.s$Biomass[be.sar.s$Stratum %in% c(2:4)]), big.mark=",", digits = 5)` t or `r prettyNum(sum(be.sar.s$Biomass[be.sar.s$Stratum %in% c(2:4)])/sum(be.sar.s$Biomass)*100, big.mark=",", digits = 1)`% in U.S. waters. -->

\newpage

(ref:biomass-ts-line) Estimated biomasses ($t$) of the five most abundant CPS in the CCE, between **Cape Flattery and San Diego [however, landmarks vary dramatically between years and it may be difficult to tease out results that conform to a common survey area; may be more expeditious to add a caveat that it's "estimated from summer ATM surveys"]**, during summer since 2008. Error bars are 95% confidence intervals.
...
(ref:biomass-ts-bar) Cumulative biomass ($t$) for the five most abundant CPS in the CCE during summer since 2008 **[see comment above regarding a common survey area]**.
