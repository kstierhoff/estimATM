...
author:
  ...
  - J.P. Zwolinski, UCSC, Cooperative Institute for Marine, Earth, & Atmospheric Systems (CIMEAS)
...
The planned core survey region, which was to be sampled by NOAA ship _`r survey.vessel.long`_ (hereafter, _`r survey.vessel`_) and two wind-powered uncrewed surface vehicles (Explorer USVs; Saildrone, Inc.), spanned most of the continental shelf between `r survey.landmark.n` and `r survey.landmark.s`. However, after losing roughly half of the scheduled sea days due to insufficient staffing aboard _`r survey.vessel`_, the plan was modified for chartered fishing vessel _Lisa Marie_ to survey _`r survey.vessel`_'s transects between `r survey.landmark.n` Cape Mendocino using 20 nmi spacing. Both _Lisa Marie_ and _`r survey.vessel`_ sampled in the area between Cape Flattery and Bodega Bay, and then _`r survey.vessel`_ sampled farther south, ending at Punta Baja, Baja CA, MX. From Bodega Bay to San Diego, CA, _`r survey.vessel`_’s sampling was augmented with nearshore sampling from fishing vessel _Long Beach Carnage_. 

North of Cape Mendocino, where _`r survey.vessel`_ did not sample, species composition and CPS length distributions were estimated from the catches of _Lisa Marie_'s daytime purse seine catches, but adjusted to reflect the associations between Pacific Sardine and Jack Mackerel in this region during summer 2018-2021 (see **Section \@ref(results-seine-catch-lm)**). Between Cape Mendocino and Punta Baja, species composition and CPS length distributions were estimate, as usual, by the catches from nighttime surface trawls.  

<!-- Throughout the core region, _`r survey.vessel`_, _Lisa Marie_, and the two USVs sampled along transects oriented approximately perpendicular to the coast, from the shallowest navigable depth (~25 m) to either a distance of 35 nmi or to the 1,000 ftm (~1830 m) isobath, whichever is farthest. In the SCB, transects in the core region were extended to approximately 75 nmi.  -->

Because sampling by _`r survey.vessel`_ and USVs in water shallower than ~20 m was deemed inefficient, unsafe, or both, fishing vessels _Lisa Marie_ and _Long Beach Carnage_ were used to estimate the biomasses of CPS in the U.S. nearshore region. The vessels sampled along 2 to 5 nmi-long transects spaced 5 or 10 nmi apart off the mainland coast of the U.S., between `r survey.landmark.n` and San Diego, CA, as well as around Santa Cruz and Santa Catalina Islands in the Southern CA Bight. In the nearshore region, the species composition and CPS length distributions were estimated using catches from daytime purse-seine sets by the fishing vessels or nighttime surface trawls by _`r survey.vessel`_ at night, whichever was nearest to the acoustically sampled CPS.  
...
The estimated biomass of the southern stock of Pacific Sardine in the survey region was `r prettyNum(be.all.summ$Biomass[be.all.summ$Species == "Sardinops sagax" & be.all$Stock == "Southern"], big.mark=",", digits = 5)` t (CI~95%~ = `r prettyNum(be.all.summ$lower.ci.B[be.all.summ$Species == "Sardinops sagax" & be.all.summ$Stock == "Southern"], big.mark=",",digits=5)` - `r prettyNum(be.all.summ$upper.ci.B[be.all.summ$Species == "Sardinops sagax" & be.all.summ$Stock == "Southern"],big.mark=",",digits=5)` t). In summer 2021 **[REVISE CALCULATIONS]**, `r prettyNum(sum(be.sar.s$Biomass[be.sar.s$Stratum %in% c(2:4)]), big.mark=",", digits = 5)` t (strata 2-4; `r prettyNum(sum(be.sar.s$Biomass[be.sar.s$Stratum %in% c(2:4)])/sum(be.sar.s$Biomass)*100, big.mark=",", digits = 1)`% of the total biomass) of southern stock biomass was observed in U.S. waters, and the remaining `r prettyNum(sum(be.sar.s$Biomass[be.sar.s$Stratum ==1]), big.mark=",", digits = 5)` t (stratum 1; `r prettyNum(sum(be.sar.s$Biomass[be.sar.s$Stratum ==1])/sum(be.sar.s$Biomass)*100, big.mark=",", digits = 1)`% of the total biomass) was observed off Baja CA. The southern stock was first observed in U.S. waters by the SWFSC's ATM surveys in 2016 [`r prettyNum(biomass.pct.sar.s$biomass[biomass.pct.sar.s$year == 2016],big.mark=",", digits = 1)` t, @Stierhoff2021b]. Southern stock biomass in U.S. waters was `r prettyNum(biomass.pct.sar.s$biomass[biomass.pct.sar.s$year == 2018],big.mark=",", digits = 1)` t in summer 2018 [@Stierhoff2019b], and was `r prettyNum(biomass.pct.sar.s$biomass[biomass.pct.sar.s$year == 2019],big.mark=",", digits = 1)` t in summer 2019 [@Stierhoff2020b].    
