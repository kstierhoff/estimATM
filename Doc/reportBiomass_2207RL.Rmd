---
title: "DISTRIBUTION, BIOMASS, AND DEMOGRAPHY OF COASTAL PELAGIC FISHES IN THE CALIFORNIA CURRENT ECOSYSTEM DURING SUMMER 2022 BASED ON ACOUSTIC-TRAWL SAMPLING"
author: 
...
save.figs        <- T # Save figures during processing
...
In `r tolower(survey.season)` `r survey.year`, the ATM survey performed in U.S. waters aboard _`r survey.vessel`_ and fishing vessel _Lisa Marie_ was augmented with coordinated sampling by fishing vessel _Long Beach Carnage_ to estimate the biomasses of CPS in nearshore regions, where sampling by _`r survey.vessel`_ was not possible or safe. In addition, two wind-powered uncrewed surface vessels (USVs; Saildrone, Inc.) conducted acoustic sampling along adaptive transects not sampled by _`r survey.vessel`_.    

Presented here are:  1) a detailed description of the ATM used to survey CPS in the CCE off the west coast of North America; and 2) estimates of the abundances, biomasses, size structures, and distributions of CPS, specifically the northern and southern stocks of Pacific Sardine; the central and northern stocks of Northern Anchovy, Pacific Mackerel, Jack Mackerel, and Pacific Herring for the core and nearshore survey regions. Additional details about the survey may be found in the survey report **[Renfree2023, in prep]**.  

**[Update permit numbers] This survey was conducted with the approval of the Secretaria de Relaciones Exteriores (SRE, Diplomatic note CTC/1312/2021), the Instituto Nacional de Estadística y Geografía (INEGI; Permit: EG0082021), and the Comisión Nacional de Acuacultura y Pesca (CONAPESCA; Permit: PPFE/DGOPA-073/21).**  
...
The `r tolower(survey.season)` `r survey.year` survey was conducted principally using _`r survey.vessel`_ and _Lisa Marie_. The sampling domain, between `r survey.landmark.n` and `r survey.landmark.s`, was defined by the conceptual distribution of potential habitat for the northern stock of Pacific Sardine at the beginning of the survey (**Fig. \@ref(fig:sardine-distribution)**), and information recently gathered from other research projects [e.g., California Cooperative Oceanic Fisheries Investigations (CalCOFI^[https://calcofi.org/]) samples] or the fishing industry (e.g., catch and bycatch data). East to west, the sampling domain extends from the coast to at least the 1,000 ftm (~1830 m) isobath (**Fig. \@ref(fig:survey-plan)**). This area encompassed the anticipated distributions of the northern and southern stocks of Pacific Sardine (**Fig. \@ref(fig:sardine-potential-habitat)**) and the central and northern stocks of Northern Anchovy off the west coasts of the U.S., and Mexico, but also spanned portions of the Pacific Mackerel, Jack Mackerel, and Pacific Herring populations. Considering the expected distribution of the target species, the acceptable uncertainty in biomass estimates, and the available ship time (`r survey.das` planned days at sea, DAS), the principal survey objectives were the estimations of biomasses for the northern and southern stocks of Pacific Sardine and the northern and central stocks of Northern Anchovy in the survey region. Additionally, biomass estimates were sought for Pacific Mackerel, Jack Mackerel, and Pacific Herring in the survey region.  
...
To increase the spatial sampling resolution, acoustic sampling was conducted by USVs (SD-1076 and SD-1077) on transects interstitial to _`r survey.vessel`_ transects from Newport, OR to San Francisco, CA (cyan lines, **Fig. \@ref(fig:survey-plan)**).  

To estimate the abundances and biomasses of CPS close to shore, in shallow water, or both, where _`r survey.vessel`_ could not efficiently or safely navigate or trawl, acoustic and purse seine sampling was conducted nearshore between Bodega Bay and San Diego using fishing vessel _Long Beach Carnage_ equipped with multi-frequency echosounders (magenta lines, **Fig. \@ref(fig:survey-plan)**). _Long Beach Carnage_ sampled 5-nmi-long transects spaced 5 nmi apart between Bodega Bay and San Diego, and 2.5-nmi-long transects spaced 2.5 nmi apart around Santa Cruz and Santa Catalina Islands in the Southern CA Bight.  **[Revise figure, to show modified planned transects by LM and Saildrone, or revise text here.]**  

(ref:survey-plan) Planned compulsory transects sampled by _`r survey.vessel`_ and _Lisa Marie_; interstitial transects sampled by USVs (cyan lines); and nearshore transects sampled by _Long Beach Carnage_ (magenta lines). Isobaths (light gray lines) are 50, 200, 500, and 2,000 m (or approximately 27, 109, 273, and 1,094 ftm, respectively).
...
(ref:sardine-potential-habitat) **[Update caption and colors]** Distribution of potential habitat for the northern stock of Pacific Sardine, temporally aggregated using an average of the habitat centered ±2° around the positions of _`r survey.vessel`_ (black lines), _Lisa Marie_ (green lines), and USVs (cyan lines; ) throughout the survey. Areas in white correspond to no available data, e.g., cloud coverage preventing satellite-sensed observations.
...
On the two USVs (SD-1076 and SD-1077; Saildrone, Inc.), miniature Wide-Bandwidth Transceivers (Simrad WBT-Mini; Kongsberg) were configured with gimbaled, keel-mounted, dual-frequency transducers (Simrad ES38-18|200-18C; Kongsberg) containing a split-beam 38-kHz transducer and single-beam 200-kHz transducer with nominally 18$^\circ$ beamwidths.  
...
For the two USVs, the echosounders were calibrated by Saildrone, Inc. while dockside in Alameda, CA, using a WC38.1 standard sphere. The data were processed and results were derived by the SWFSC [@Renfree2019], and are presented in **Table \@ref(tab:cal-results-sd)**.
...
```{r cal-results-sd,results='asis',eval=FALSE}
if (doc.type == "docx") {
  # create kable object (for Word)
...
```{r trawl-diagram-carranza, fig.cap='(ref:trawl-diagram-carranza)',out.width='6.5in',fig.align='center',fig.pos='H',eval=FALSE}
...
\
The total estimated biomass of the northern stock of Northern Anchovy was `r prettyNum(be.all$Biomass[be.all$Species == 'Engraulis mordax' & be.all$Stock == "Northern"],big.mark=",",digits=3)` t (CI~95%~ = `r prettyNum(be.all$lower.ci.B[be.all$Species == 'Engraulis mordax' & be.all$Stock == "Northern"],big.mark=",",digits=3)` - `r prettyNum(be.all$upper.ci.B[be.all$Species == 'Engraulis mordax' & be.all$Stock == "Northern"],big.mark=",",digits=3)` t, CV = `r prettyNum(be.all$biomass.cv[be.all$Species == 'Engraulis mordax' & be.all$Stock == "Northern"],big.mark=",",digits=1)`%; **Table \@ref(tab:biomass-anch-n)**), all in the core region. The stock was distributed throughout the survey area from approximately Westport to Cape Mendocino (**Fig. \@ref(fig:biom-dens-anch-n)a**). $L_S$ ranged from `r length.summ$L.min[length.summ$scientificName == 'Engraulis mordax' & length.summ$stock == "Northern"]` to `r length.summ$L.max[length.summ$scientificName == 'Engraulis mordax' & length.summ$stock == "Northern"]` cm with a single mode at 14 cm (**Table \@ref(tab:l-freq-summ-anch-n)**, **Fig. \@ref(fig:l-disagg-anch-n)**). $L_S$ had a single mode at 11 cm (**Table \@ref(tab:l-freq-summ-anch-n)**, **Fig. \@ref(fig:l-disagg-anch-n)**).  
...
(ref:biom-dens-anch-n) Biomass densities of the northern stock of Northern Anchovy (_Engraulis mordax_), per stratum, in the core survey region. The blue numbers represent the locations of trawl clusters with at least one Northern Anchovy. The gray line represents the vessel track.

```{r biom-dens-anch-n,fig.cap='(ref:biom-dens-anch-n)',out.width='6.5in',fig.pos='H'}
# No NSNA in the nearshore region in summer 2022
...
(ref:l-disagg-anch-n) Abundance versus standard length ($L_S$, upper panels) and biomass (t) versus $L_S$ (lower panels) for the northern stock of Northern Anchovy (_Engraulis mordax_) in the core survey region.
...
# No herring in the nearshore region in summer 2022.
...
```{r biom-dens-rher,fig.cap='(ref:biom-dens-rher)',out.heigh='7in',fig.pos='H',eval=FALSE}
...
**[Document catastrophic losses of sea days and list 1) impacts to the survey and 2) mitigation measures take during the survey and afterward during analysis.]**
...
```{r sardine-stock-allocation,fig.cap='(ref:sardine-stock-allocation)',out.height='8in',fig.pos='H',eval=FALSE}
...
**[Update time series plot, including 2021 results and checking species colors/values; e.g., jack mackerel missing]**
