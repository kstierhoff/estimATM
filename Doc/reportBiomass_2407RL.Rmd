...
```{r summarise-seine-catch,eval=FALSE}
...
The nearshore region spanned an area from 5-m depth to approximately 5 nmi from the continental coast, or 2.5 nmi from the Santa Cruz and Santa Catalina Islands, between Cape Flattery and San Diego. _Lisa Marie_ (`das.lm` DAS) surveyed from approximately Cape Flattery, WA to Bodega Bay with `nrow(nasc.summ.lm.ns)` east-west transects totaling `sprintf("%.0f",sum(nasc.summ.lm.ns$distance))` nmi and `set.summ$n[set.summ$vessel.name == "LM"]` purse-seine sets. _Long Beach Carnage_ (`das.lbc` DAS) surveyed from approximately Bodega Bay to San Diego, and around the Santa Cruz and Santa Catalina Islands, with `nrow(nasc.summ.lbc.ns)` east-west transects totaling `sprintf("%.0f",sum(nasc.summ.lbc.ns$distance))` nmi and `set.summ$n[set.summ$vessel.name == "LBC"]` purse-seine sets (**Fig. \@ref(fig:nasc-seine-lbc)**). In the nearshore region, one to `num2words(max(as.numeric(be.ns$Stratum),na.rm = T))` post-survey strata were defined by their transect spacing and the densities of biomass attributed to each species.
...
Northern Anchovy and Pacific Sardine were predominant, by weight, in purse-seine samples collected by _Long Beach Carnage_ in the nearshore region (**Fig. \@ref(fig:nasc-seine-lbc)b**). In general, Northern Anchovy were more abundant north of Pt. Conception and Pacific Sardine were more abundant to the south (**Fig. \@ref(fig:nasc-seine-lbc)b**). Pacific Mackerel were collected in several purse-seine samples along the mainland coast in the SCB. Few Jack Mackerel were collected in purse-seine samples collected by _Long Beach Carnage_ (**Fig. \@ref(fig:nasc-seine-lbc)b**). Overall, dip-net samples from `set.summ$n[set.summ$vessel.name == "LBC"]` seines totaled `prettyNum(sum(seine.summ.lbc$total.wt),big.mark=",",digits=3)` kg of CPS (`prettyNum(seine.summ.lbc$total.wt[seine.summ.lbc$scientificName == "Sardinops sagax"],big.mark=",",digits=3)` kg of Pacific Sardine, `prettyNum(seine.summ.lbc$total.wt[seine.summ.lbc$scientificName == "Scomber japonicus"],big.mark=",",digits=3)` kg of Pacific Mackerel, `prettyNum(seine.summ.lbc$total.wt[seine.summ.lbc$scientificName == "Engraulis mordax"],big.mark=",",digits=2)` kg of Northern Anchovy, and `prettyNum(seine.summ.lbc$total.wt[seine.summ.lbc$scientificName == "Trachurus symmetricus"],big.mark=",",digits=2)` kg of Jack Mackerel; no Pacific Herring were caught).  
