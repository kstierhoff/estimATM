...
pacman::p_load(tidyverse,swfscMisc,lubridate,sp,rgeos,mapview,RODBC,
               knitr,geosphere,ggrepel,cowplot,sf,leaflet, htmltools,
               odbc,kableExtra,rnaturalearth,shadowtext,here,fs,
...
```{r extract-catch}
  ...
  mutate(county = NA,
         ...
         blank1 = NA,
         blank2 = NA) %>% 
  arrange(desc(totalCt)) %>% 
  select(commonName, blank1, scientificName, blank2, county, MPA, 
...
specimen.summ <- specimens.ca %>% 
  ...
  filter(!is.na(otolithNumber) | !is.na(DNAvialNumber) | 
           !is.na(weightg)) %>% 
  select(commonName, scientificName, weightg, otolithNumber, DNAvialNumber, otolithNumber) %>% 
...
```{r catch-summary}
  ...
  select(commonName, scientificName, MPA, long, lat, totalCt, disposition) %>% 
...
```{r specimen-summary}
datatable(specimen.summ, rownames = FALSE)
```
