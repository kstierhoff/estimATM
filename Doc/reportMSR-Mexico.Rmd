...
if (get.nav) {
  ...
  if (file.exists(here("Data/Nav/nav_data_msr.Rdata"))) {
    load(here("Data/Nav/nav_data_msr.Rdata"))
  ...
  if (nav.lag > 24) {
    ...
    nav.temp <- data.frame(read.csv(dataURL, header = TRUE,  
                                    row.names = NULL, skip = 0)) 
    # names(nav.temp) <- erddap.headers
    
    # Filter to remove bad SST values
    nav.temp <- nav.temp %>% 
      mutate(long     = longitude..degrees_east. - 360,
             lat      = latitude..degrees_north.,
             SOG      = platformSpeed..meter.second.1. * 1.94384,
             datetime = ymd_hms(time..UTC.),
             leg      = paste("Leg", cut(as.numeric(date(time..UTC.)), 
                                         leg.breaks, labels = F))) %>%
      filter(is.nan(SOG) == F, SOG > 0, SOG < 15,
             between(latitude..degrees_north., min(survey.lat), max(survey.lat)), 
  ...
  # Add ID to nav data, to facilitate filtering of data in Mexico
  nav$nav_id <- seq_along(nav$ID)
  ...
  # Remove nav data outside of Mexico
  nav <- filter(nav, nav_id %in% nav.sf$nav_id)
  
  # Save results
  save(nav, nav.sf, nav.paths.sf, file = here("Data/Nav/nav_data_msr.Rdata"))
} else {
  # Load nav data
  load(here("Data/NAV/nav_data_msr.Rdata"))
}
...
nav.now <- tail(nav, 1) %>%
  st_as_sf(coords = c("long","lat"), crs = crs.geog) %>% 
  mutate(label = paste("Last position:", time..UTC., "UTC"))
```
...
ggsave(base.map.bw,
       file = here("Figs/fig_basemap_mex_bw.png"), 
       height = map.height, width = map.width)
```
...
if (do.spatial) {
  ...
  save(haul.mex, file = here("Output/haul_data_reportMSR-Mexico.Rdata"))
} else {
  load(here("Output/haul_data_reportMSR-Mexico.Rdata"))
}
...
write_csv(catch.summary,
          here("Output/catch_summary_MSR-Mexico.csv"),
          na = "")
```
...
imap.bounds <- st_bbox(nav.sf) 
imap.bounds1 <- map_bounds(nav$lat, nav$long, 0.1)
...
if (nrow(catch.mex) > 0) {
  ...
  write_csv(catch.all,
            here("Output/catch_table_MSR-Mexico.csv"),
            na = "") 
} else {
  cat("No catch in Mexico")
}
...
```{r export-data}
fs::dir_create(here("Output/MSR"))

# Meteorological data
write_csv(nav, file = here("Output/MSR", "meteorological_data.csv"))

# Trawl data
write_csv(haul, file = here("Output/MSR", "trawl_haul_data.csv"))
write_csv(catch.mex, file = here("Output/MSR", "trawl_catch_data.csv"))
write_csv(lengths.mex, file = here("Output/MSR", "trawl_specimen_data.csv"))

# CTD data
write_csv(ctd.summary, file = here("Output/MSR", "ctd_cast_summary.csv"))
write_csv(ctd.casts,   file = here("Output/MSR", "ctd_cast_data.csv"))

# UCTD data
write_csv(uctd.summary, file = here("Output/MSR", "uctd_cast_summary.csv"))
write_csv(uctd.casts,   file = here("Output/MSR", "uctd_cast_data.csv"))

# Backscatter data
write_csv(nasc.mex,   file = here("Output/MSR", "backscatter_data.csv"))
```
