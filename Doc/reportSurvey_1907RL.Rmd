...
During `r tolower(survey.season)` `r survey.year`, the west coasts of the United States adn Vancouver Island, Canada, were surveyed using _`r survey.vessel`_. Compulsory transects were nearly perpendicular to the coast with separations of 10 to 20 nmi. The survey began off `r survey.landmark.n`, and progressed southwards toward `r survey.landmark.s`. Irrespective of the size of the stock and the extent of its migration, the northern stock of Pacific Sardine resides within its potential habitat [@Zwolinski2011].  
...
(ref:survey-plan) Planned compulsory (solid black lines) and adaptive (dashed red lines) transect lines; offshore extensions to compulsory acoustic transects (dashed green lines); and nearshore transect lines (solid magenta lines). Isobaths (light gray lines) are placed at 50, 200, 500, and 2,000 m (or approximately 25, 100, 250, and 1,000 fathoms).
...
First, the transducer integrities were verified through transducer-impedance measurements in water and air using an LCR meter (Agilent E4980A) and custom Matlab software. The impedance magnitude ($|Z|$, $\Omega$), phase ($\theta$, $^\circ$), conductance ($G$, $S$), susceptance ($B$, $S$), resistance ($R$, $\Omega$), and reactance ($X$, $\Omega$) were measured at the operational frequencies for each transducer with its quadrants connected in parallel (**Appendix \@ref(appendix-impedance-analyisis)**).  

Next, the echosounder systems were calibrated using the standard sphere technique [@Foote1987;@Demer2015]. The reference target was a `r cal.sphere` (`r cal.sphere.name`). The GPTs and WBTs were configured using the calibration results via the ER60 and EK80 software, respectively (see **Section \@ref(results-echosounder-calibration)**).  
...
The EK80s were calibrated between `r cal.datetime` while the vessel was alongside the pier near `r cal.loc` (`r cal.lat.dd` $^\circ$N, `r cal.lon.dd` $^\circ$W). Measurements of sea-surface temperature ($t_w$ = `r cal.temp` $^\circ$C) and salinity ($s_w$ = `r cal.sal` psu) to a depth of 10 m were measured using a handheld probe (Pro2030, YSI) and input to the WBT-control software (EK80 `r ek80.version`, Simrad), which derived estimates of sound speed ($c_w$ = `r cal.c` m s^-1^) and absorption coefficients (see **Table \@ref(tab:cal-results)**). Varying with tide, the seabed was approximately `r cal.min.z` to `r cal.max.z` m beneath the transducers. The calibration sphere (`r cal.sphere`) was positioned nominally 5-8 m below the transducers.  

WBT information, configuration settings, and beam model results following calibration are presented in **Table \@ref(tab:cal-results)**. Measurements of uncompensated sphere target strength ($TS_u$, dB re 1 m^2^) are plotted in **Fig. \@ref(fig:tsu-plot)** and beam-compensated sphere target strength ($TS_{rel}$, dB re 1 m^2^), relative to the theoretical target strength, are plotted in **Fig. \@ref(fig:tsc-plot)**. During the calibration with the centerboard in the retracted position, one quadrant of the 18-kHz transducer appeared to be dysfunctional. An examination of impedance measurements made during the survey with the centerboard in the extended position indicated that the transducer may have been functional during the survey (**Appendix \@ref(appendix-impedance-analyisis)**). Therefore, the results of the last known good calibration of the 18-kHz transducer, from the 2018 Summer CCE survey, are presented in **Table \@ref(tab:cal-results)**. A time-series of calibration results for _`r survey.vessel`_, including on-axis gain ($G_0$), $S_a$ correction ($S_\mathrm{a}\mathrm{corr}$), beamwidths ($\alpha_{-3dB}$ and $\beta_{-3dB}$), offset angles ($\alpha_0$ and $\beta_0$), and RMS, are plotted in **Fig. \@ref(fig:cal-ts-plot)**.  
...
(ref:cal-results) Simrad EK60 general purpose transceiver (GPT, 18- and 38-kHz) and EK80 wideband transceiver (WBT; 70-, 120-, 200-, and 333-kHz) information, pre-calibration settings (above horizontal line), and beam model results following calibration (below horizontal line). Prior to the survey, on-axis gain ($G_0$), beam angles and angle offsets, and $S_a$ Correction ($S_\mathrm{a}\mathrm{corr}$) values from calibration results were entered into the GPT- and WBT-control software (ER60 and EK80, respectively; Simrad).
...
Pacific Herring comprised the greatest proportion of catch in trawl samples, by weight, in Canadian waters and in nearshore U.S. waters north of the Columbia River (**Fig. \@ref(fig:nasc-cufes-trawl)c**). Jack Mackerel comprised the greatest proportion of catch in trawl samples between approximately Newport and Bodega Bay (**Fig. \@ref(fig:nasc-cufes-trawl)c**). Northern Anchovy were predominant in trawls conducted inshore and offshore between Bodega Bay and San Diego. Relatively small amounts of Pacific Sardine were collected in trawls conducted between the Columbia River and Cape Mendocino; near Bodega Bay; and around the northern Channel Islands in the Southern CA Bight (**Fig. \@ref(fig:nasc-cufes-trawl)c**). Some Pacific Mackerel were collected along the OR and northern CA coast; and offshore near Bodega Bay and Pt. Conception (**Fig. \@ref(fig:nasc-cufes-trawl)c**). Overall, the `r nrow(trawl.summ)` trawls captured a combined `r sprintf("%.0f",haul.CPS.kg)` kg of CPS (`r sprintf("%.0f",haul.Sardine.kg)` kg Pacific Sardine, `r sprintf("%.0f",haul.Anchovy.kg)` kg Northern Anchovy, `r sprintf("%.0f",haul.JackMack.kg)` kg Jack Mackerel, `r sprintf("%.0f",haul.PacMack.kg)` kg Pacific Mackerel, and `r sprintf("%.0f",haul.PacHerring.kg)` kg Pacific Herring; **Appendix \@ref(appendix-trawl-sampling)**).  

Off Vancouver Island and nearshore off WA, north of the Columbia River, Pacific Herring catches were the largest proportions in trawl samples, by weight, (**Fig. \@ref(fig:nasc-cufes-trawl)c**). Between approximately Newport and Bodega Bay, Jack Mackerel dominated the trawl catches (**Fig. \@ref(fig:nasc-cufes-trawl)c**). Inshore and offshore, between Bodega Bay and San Diego, Northern Anchovy were predominant species in the trawl catches. Between the Columbia River and Cape Mendocino, near Bodega Bay, and around the northern Channel Islands in the SCB, Pacific Sardine were caught in relatively small numbers (**Fig. \@ref(fig:nasc-cufes-trawl)c**). Along the OR and northern CA coasts, and offshore near Bodega Bay and Pt. Conception, a few Pacific Mackerel were caught (**Fig. \@ref(fig:nasc-cufes-trawl)c**). Overall, the `r nrow(trawl.summ)` trawls captured a combined `r sprintf("%.0f",haul.CPS.kg)` kg of CPS (`r sprintf("%.0f",haul.Sardine.kg)` kg Pacific Sardine, `r sprintf("%.0f",haul.Anchovy.kg)` kg Northern Anchovy, `r sprintf("%.0f",haul.JackMack.kg)` kg Jack Mackerel, `r sprintf("%.0f",haul.PacMack.kg)` kg Pacific Mackerel, and `r sprintf("%.0f",haul.PacHerring.kg)` kg Pacific Herring; **Appendix \@ref(appendix-trawl-sampling)**).  
...
+ Industry vessels extended the echosounder and net-catch data to nearshore areas where Lasker could not safely navigate or trawl. With coordination, the sampling from Lasker was concomitant with sampling from Lisa Marie off WA and OR, and from Long Beach Carnage off SCB.  

## Disadvantages  
+ The USV sampling nearshore acoustic transects where wind was light and variable had difficulty navigating and progressed slowly compared to the offshore USVs, and had difficulty sampling coincidently with _`r survey.vessel`_;
...
In summer 2018, _`r survey.vessel`_ only surveyed in water depths greater than ~20 to 30 m, and therefore potentially under-sampled any CPS aggregations in the shallower, nearshore area. In summer 2019, collaborative research was conducted contemporaneously to quantify this potential sampling bias by using two industry fishing vessels, F/V _Lisa Marie_ and F/V _Long Beach Carnage_, and one USV to extend the sampling closer to shore. Acoustic and purse seine sampling was conducted from these two vessels along the coast of WA and OR; and along the mainland coast and around the northern Channel Islands in the Southern CA Bight. Acoustic-only sampling was conducted from the nearshore USV between Cape Flattery and Pt. Conception. The methods and preliminary results from those surveys are presented here.  
...
During Leg I, _`r survey.vessel`_ coordinated sampling with F/V _Lisa Marie_, which conducted daytime acoustic and purse seine sampling in shallow, nearshore waters (within ~ 5 nmi of the coast) between Cape Flattery, WA and the OR/CA border. Aboard _Lisa Marie_, SWFSC’s split-beam echosounder (Simrad EK60 GPT) was connected to the vessel's 38 kHz transducer (Simrad ES38-B), which was calibrated while on anchor in Grays Harbor (46.9236, -124.1181) on 8 May. Transmit pulses from the echosounder and sonar were not synchronized.  

From 20 June to 1 July, _Lisa Marie_ conducted echosounder-sonar transects between the eastern ends of _`r survey.vessel`_’s acoustic transects and shore. Each day, approximately **two** purse seine sets were conducted to provide information about size, age, and species composition. All specimens were frozen and later processed by the Washington Department of Fish and Wildlife (WA-DFW). On 21 June, Greg Shaughnessy (Ocean Gold Seafoods) embarked _`r survey.vessel`_ to observe survey operations. At the same time, Josiah Renfree boarded _Lisa Marie_ to remedy minor issues with her echosounder before rejoining _`r survey.vessel`_. At ~1900 on 25 June, Mr. Shaughnessy disembarked and was put ashore in Westport, WA via _`r survey.vessel`_'s skiff.  

#### F/V _Long Beach Carnage_ {#appendix-nearshore-acoustic-sampling-carnage}  

During Leg IV, _`r survey.vessel`_ coordinated sampling with F/V _Long Beach Carnage_, which conducted daytime acoustic and purse seine sampling in shallow, nearshore waters (within ~ 5 nmi of the coast) in the Southern CA Bight between Pt. Conception and San Diego and around the northern Channel Islands. Aboard _Long Beach Carnage_, the SWFSC's multi-frequency EK60 GPTs (70, 120, 200, and 333 kHz; Simrad) were configured with the SWFSC's split-beam multi-frequency transducer array (MTA4, ES38-12, ES70-7C, ES120-7C and ES200-7C; Simrad) mounted on the bottom of a pole. The transducers were calibrated on 18 May in the SWFSC's Ocean Technology Development Tank. Transmit pulses from the echosounder and sonar were not synchronized.  

From 26 August to **6** September, _Long Beach Carnage_ conducted echosounder-sonar transects between the eastern ends of _`r survey.vessel`_’s acoustic transects and shore. Each day, approximately **two** purse seine sets were conducted to provide information about size, age, and species composition. **All specimens were [DESCRIBE SAMPLE PROCESSING PROCEURES, IF ANY].**  

#### Unmanned surface vehicles {#appendix-nearshore-acoustic-sampling-saildrone}  

From **[START DATE]** to **[END DATE]**, one USV (Saildrone 1047) conducted daytime acoustic sampling in shallow, nearshore waters between Cape Flattery and Pt. Conception. **[Describe areas where transects were skipped]**

### Purse seine sampling {#appendix-nearshore-net-sampling}
#### F/V _Lisa Marie_ {#appendix-nearshore-net-sampling-lisa-marie}

For each set, three dip net samples separated as much as possible were collected. For each dip net sample, Pacific Sardine, Northern Anchovy, Jack Mackerel, Pacific Mackerel, and Pacific Herring were sorted, weighed, an counted to provide a combined weight and count for each species. Next, all three dip net samples were combined and up to 50 specimens were randomly sampled to provide a combined weight for each set.

The length ($L_S$ for Pacific Sardine and Northern Anchovy and $L_F$ for all others; mm) and weight (g) were measured. for up to 25 specimens selected randomly from each species, otolithes were extracted, macroscopic maturity stage was determined visually, and gonads were collected and preserved from females.  

#### F/V _Long Beach Carnage_ {#appendix-nearshore-net-sampling-carnage}

**[Same methods as Lisa Marie?]**
