...
The survey spanned an area from approximately `r survey.landmark.n`, to `r survey.landmark.s` (**Fig. \@ref(fig:station-sampling)**), with `r nrow(nasc.summ)` east-west transects totaling `r sprintf("%.0f",sum(nasc.summ$distance))` nmi, and `r nrow(trawl.summ)` Nordic trawls. Two USVs (SD-1045 and SD-1046) surveyed the offshore area between Florence, OR and San Diego, with `r n_distinct(nasc.summ.sd.os$transect)` east-west transects totaling `r sprintf("%.0f",sum(nasc.summ.sd.os$distance))` nmi.  
