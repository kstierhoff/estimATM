...
General purpose transceiver (EK60 GPT, Simrad) beam model results were estimated from a dockside calibration using a WC38.1. Calibration parameters were derived using the following methods: (1) the two excursions of the sphere throughout the beam were processed separately using the EK80 software, (2) both excursions combined and processed using the EK80 software, and (3) both excursions combined and processed using Echoview and Matlab. On-axis gain ($G_0$), beam angles and angle offsets, and $S_a$ Correction ($S_\mathrm{a}\mathrm{corr}$) values from Excursion 2 were applied in Echoview during post-processing.  
...
General purpose transceiver (EK60 GPT, Simrad) beam model results were estimated from a tank calibration using a WC38.1. Prior to the survey, calibrated on-axis gain ($G_0$), beam angles and angle offsets, and $S_a$ Correction ($S_\mathrm{a}\mathrm{corr}$) values were entered into the GPT-control software (EK80, Simrad).  
...
Wideband transceiver (EK80 WBT-Mini, Simrad) beam model were estimated from a dockside calibration conducted by Saildrone, Inc. Calibrated on-axis gain ($G_0$), beam angles and angle offsets, and $S_a$ Correction ($S_\mathrm{a}\mathrm{corr}$) values were applied in Echoview during post-processing.  
