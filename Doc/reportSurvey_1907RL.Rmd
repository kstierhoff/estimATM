...
The combined use of NOAA ships, industry fishing vessels, and USVs to conduct ATM surveys of CPS identified several advantages and disadvantages, which are elaborated below from the perspective of the SWFSC's Advanced Survey Technologies Group. 
...
+ Industry vessels extended the echosounder and net-catch data to nearshore areas where _`r survey.vessel`_ could not safely navigate or trawl. With coordination, the sampling from _`r survey.vessel`_ was concomitant with sampling from _Lisa Marie_ off WA and OR, and from _Long Beach Carnage_ off SCB.  

## Disadvantages  
+ The USV sampling nearshore acoustic transects where wind was light and variable had difficulty navigating and progressed slowly compared to the offshore USVs, and therefore had decreased coincident sampling with _`r survey.vessel`_;
+ The industry vessels and USVs sampled with fewer echosounder frequencies than _`r survey.vessel`_. Consequently, there were differences in the acoustic data processing  which may affect the identification of CPS echoes.  
...
In summer 2018, _`r survey.vessel`_ only surveyed in water depths greater than ~20 to 30 m, and therefore potentially under-sampled any CPS aggregations in the shallower, nearshore area. In summer 2019, collaborative research was conducted contemporaneously to quantify this potential sampling bias by using two industry fishing vessels, F/V _Lisa Marie_ and F/V _Long Beach Carnage_, and one USV to extend the sampling closer to shore. The methods and preliminary results from those surveys are presented here.  
...
From 20 June to 24 August, one USV (Saildrone 1047) conducted daytime acoustic sampling in shallow, nearshore waters between Cape Flattery and Pt. Conception. **[Describe areas where transects were skipped]**
...
Wideband transceiver (EK80 WBT, Simrad) beam model results following calibration. Prior to the survey, on-axis gain ($G_0$), beam angles and angle offsets, and $S_a$ Correction ($S_\mathrm{a}\mathrm{corr}$) values from calibration results were entered into the GPT-control software (EK80, Simrad).  
...
F/V _Lisa Marie_ surveyed the nearshore area from approximately Cape Flattery the WA-OR border (**Fig. \@ref(fig:nasc-seine-lm)**), with `r nrow(nasc.summ.lm)` east-west transects totaling `r sprintf("%.0f",sum(nasc.summ.lm$distance))` nmi, and `r nrow(lm.sets)` purse seine sets. F/V _Long Beach Carnage_ surveyed the nearshore area from approximately Pt. Conception to San Diego, including the nearshore areas around Santa Cruz and Santa Catalina Islancs  (**Fig. \@ref(fig:nasc-seine-lbc)**), with `r nrow(nasc.summ.lbc)` east-west transects totaling `r sprintf("%.0f",sum(nasc.summ.lbc$distance))` nmi, and `r nrow(lbc.sets)` purse seine sets. **[Add Saildrone survey statistics]**  

### Distribution of CPS {#appendix-nearshore-results-distribution}

Acoustic backscatter ascribed to CPS off the WA and OR coast was most prevalent near the Columbia River and around Newport (**Fig. \@ref(fig:nasc-seine-lm)a**). Acoustic backscatter ascribed to CPS was observed throughout the nearshore survey area in the SCB, but was most prevalent between Santa Barbara and Malibu; and between Costa Mesa and La Jolla (**Fig. \@ref(fig:nasc-seine-lbc)a**). **[Add Saildrone acoustic sampling results]**.

Pacific Herring catches were predominant, by weight, in purse seine samples collected between Cape Flattery and Newport (**Fig. \@ref(fig:nasc-seine-lm)b**). Jack Mackerel dominated the purse seine catches between approximately Newport and the OR-CA border (**Fig. \@ref(fig:nasc-seine-lm)b**). Northern Anchovy were present in four purse seines set by _Lisa Marie_ and one set by _Long Beach Carnage_ (most catches not visible at this scale), but were most abundant north of the Columbia River (**Fig. \@ref(fig:nasc-seine-lm)b**). Pacific Sardine were caught in relatively small numbers between the Columbia River and Newport (**Fig. \@ref(fig:nasc-seine-lm)b**), but dominated the purse seine samples collected throughout the SCB (**Fig. \@ref(fig:nasc-seine-lbc)b**). A few Pacific Mackerel were caught in the SCB near Anacapa Island and Long Beach (**Fig. \@ref(fig:nasc-seine-lbc)b**).  

## Discussion {#appendix-nearshore-discussion}

**[REVIEW and REVISE]**  

+ Both industry vessels closely adhered to the planned transect lines.
