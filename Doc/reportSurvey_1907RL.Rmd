...
A CalCOFI bongo oblique net [a bridleless pair of 71-cm diameter nets with 505-$\mu$m mesh; @Smith1977] was used opportunistically to sample ichthyoplankton and krill after sunset. Where there was adequate depth, 300 m of wire was deployed at a rate of 50 m min^-1^ and then retrieved at 20 m min^-1^, at a nominal wire angle of 45$^\circ$. Bongo samples were stored in 5% buffered formalin.  
...
+ The USV sampled acoustic transects nearshore, where wind was often light and variable, had difficulty navigating and progressed slowly along the coasts of WA and OR compared to the offshore USVs, and therefore had decreased coincident sampling with _`r survey.vessel`_;
...
+ Researchers at NWFSC detected an erroneous timestamp in the echosounder data collected by Saildrone, but after initial data processing had been completed (see **Appendix \@ref(appendix-nearshore-acoustic-sampling-saildrone)**). Consequently, the data from the nearshore and offshore strata had to be completely reprocessed.
...
The echosounder data from the Saildrones were erroneously timestamped (UTC+7 hrs), causing the correctly timestamped (UTC) navigation data to be mismatched. The erroneous timestamps were corrected in post-processing using the Fileset Time Offset parameter in Echoview.  

#### F/V _Long Beach Carnage_ {#appendix-nearshore-acoustic-sampling-carnage}  

During Leg IV (from 26 August to 6 September), _`r survey.vessel`_ coordinated sampling with F/V _Long Beach Carnage_ in the Southern CA Bight between Pt. Conception and San Diego and around the northern Channel Islands. Aboard _Long Beach Carnage_, the SWFSC's multi-frequency echosounders (70, 120, 200, and 333 kHz EK60 GPTs; Simrad) were configured with the SWFSC's multi-frequency array (MTA4) with split-beam transducers (ES38-12, ES70-7C, ES120-7C and ES200-7C; Simrad) mounted on the bottom of a pole. The transducers were calibrated on 18 May in the SWFSC's Ocean Technology Development Tank. Transmit pulses from the echosounder and sonar were not synchronized but appeared not to interfere with each other.   

 _Long Beach Carnage_ conducted echosounder-sonar transects between the eastern ends of _`r survey.vessel`_’s acoustic transects and shore. Purse seines were set to provide information about the size, age, and species composition of fish echoes sampled by the echosounders (see **Appendix \@ref(appendix-nearshore-net-sampling)** below). All specimens were frozen and later processed by the California Department of Fish and Wildlife (CDFW).  

### Purse seine sampling {#appendix-nearshore-net-sampling}
#### F/V _Lisa Marie_ {#appendix-nearshore-net-sampling-lisa-marie}

As many as three purse seine sets were planned each day. For each set, three dip net samples, spatially separated as much as possible, were collected. For each dip net sample, Pacific Sardine, Northern Anchovy, Jack Mackerel, Pacific Mackerel, and Pacific Herring were sorted, weighed, and counted to provide a combined weight and count for each species. Next, all three dip net samples were combined and up to 50 specimens were randomly sampled to provide a combined weight for each set.
...
Calibration parameters were estimated from a dockside calibration with a WC38.1 conducted by Saildrone, Inc. Beam model results were applied in Echoview during post-processing and are presented in **Table \@ref(tab:cal-results-sd)**.

(ref:cal-results-sd) Wideband transceiver (EK80 WBT-Mini, Simrad) beam model estimated from a dockside calibration with a WC38.1. Calibrated on-axis gain ($G_0$), beam angles and angle offsets, and $S_a$ Correction ($S_\mathrm{a}\mathrm{corr}$) values were applied in Echoview during post-processing.
...
+ Species composition of purse seine samples collected by _Lisa Marie_ were similar to that in surface trawl catches from _Lasker_ off the coast of WA and OR. 
+ Species composition of purse seine samples collected by _Long Beach Carnage_ were dominated by Pacific Sardine, whereas Northern Anchovy dominated all of the trawls conducted by _Lasker_ in the same areas.

\newpage

(ref:nasc-seine-lm) Nearshore survey transects conducted by F/V _Lisa Marie_ overlaid with a) the distribution of 38-kHz integrated backscattering coefficients ($s_A$, m^2^ nmi^-2^; averaged over 2000-m distance intervals and from `r int.start`- to `r cps.depth`-m deep) ascribed to CPS and b) the proportions, by weight, of CPS in each purse seine set.
