...
if (process.scs) {
  if (scs.source == "CSV") {
    ...
    for (b in bridge.events) {
      tmp <- read_xlsx(bridge.events[b]) %>% 
        select(Date, Time, Button, Notes = notes.hdr, 
               Lat = gps.lat.hdr, Lon = gps.lon.hdr) 
        
        bridge.snap <- bind_rows(bridge.snap, tmp)
    }
...
The objectives for the survey were to: 1) acoustically map the distributions and estimate the abundances of CPS, i.e., Pacific Sardine _Sardinops sagax_, Northern Anchovy _Engraulis mordax_, Pacific Herring _Clupea pallasii_, Pacific Mackerel _Scomber japonicus_, and Jack Mackerel _Trachurus symmetricus_; and krill (euphausiid spp.); 2) characterize their biotic and abiotic environments, and investigate linkages; 3) gather information regarding their life histories; and 4) further evaluate the use of unmanned surface vehicles (USVs) and industry vessels to augment the sampling from NOAA ships, particularly in nearshore areas where sampling using the larger NOAA ships is difficult or impossible.  
...
* Leg III: E. Gardner, N. Hunter^2^, D. Hwang^2^, H. Manjebrayakath^2^, A. Thompson, L. Vasquez de Mercado, E. Weber^1^
* Leg IV:  M. Craig, A. Hays, A. Friere, D. Griffith, T. Howard^2^, K. Runge^2^
...
```{r cal-map,fig.cap='(ref:cal-map)',out.width = '5in',fig.align='center',fig.pos='H'}
...
**Leg II**  
On 8 July, _`r survey.vessel`_ departed from MOC-P Pier in Newport, OR at ~0000. Acoustic sampling resumed at ~0145 on 8 July along Transect 083 south of Newport, OR. On 24 July, acoustic sampling ceased after the completion of Transect 50 off Albion, CA. On 25 July, _`r survey.vessel`_ arrived at the Pier 30/32 in San Francisco, CA at ~1300 to complete Leg II.  

**Leg III**  

On 30 July, _`r survey.vessel`_ departed from Pier 30/32 in San Francisco, CA at ~1300 and transited to transect 049 north of the Point Arena lighthouse. Trawling was conducted the evening of 30 July prior to beginning the acoustic sampling on transect 049 on 31 July. Intermittent malfunctions of the trawl winches resulted in reduced trawl sampling from 1-2 August. More severe malfunctions of the trawl winches prohibited trawling between 3 and 6 August. On 6 August, the trawl winch was repaired and normal sampling resumed. On 16 August, acoustic sampling ceased after the completion of transect 023 off Morro Bay. On 17 August, _`r survey.vessel`_ arrived at the 10th Avenue Marine Terminal in San Diego, CA at ~1400 to complete Leg III.  

**Leg IV**

On 22 August, _`r survey.vessel`_ departed from 10th Avenue Marine Terminal in San Diego, CA at ~1500 and performed training on the ship’s dynamic-positioning system in San Diego Harbor until ~2000.  We then transited to transect 19 off Pismo Beach. On 23 August, at ~1700, _`r survey.vessel`_ deployed a benthic acoustic lander off Point Conception, then resumed acoustic sampling along transect 019 at ~1800. On 1 September, the fog horn was repaired using parts received from ashore via _`r survey.vessel`_'s skiff. At ~1500 on 1 September, the UCTD probe was lost when a small vessel running parallel to the ship made a sharp turn across the stern and severed the line. On 09 September, acoustic sampling ceased after the completion of transect 001 off San Diego and _`r survey.vessel`_ arrived at the 10th Avenue Marine Terminal in San Diego, CA at **~HHMM** to complete the survey.  

#### Nearshore sampling  

Nearshore acoustic sampling was conducted along transects ~5 nmi-long spaced ~5 nmi-apart to provide direct estimates of CPS biomass in shallow, nearshore waters where sampling by _`r survey.vessel`_ was not possible. During Legs I and IV, _`r survey.vessel`_ coordinated sampling with two industry fishing vessels, F/V _Lisa Marie_ and _Long Beach Carnage_, that conducted acoustic and purse seine sampling off the coast of WA and OR and in the Southern CA Bight, respectively. Throughout much of the survey, one USV conducted acoustic sampling between Cape Flattery, WA and San Francisco. Details of those surveys are presented in **Appendix \@ref(appendix-nearshore)**.  
...
Calibrations of the echosounders aboard the NOAA Ship _Lasker_ were conducted between 30 April and 3 May 2019. As part of the calibrations, impedance was measured of each quadrant of each transducer, with the transducers in both air (centerboard in maintenance position) and water (centerboard in retracted position, **Fig. \@ref(fig:impedance-plots)**). The impedance measurements indicated that quadrant 1 of the 18 kHz transducer (ES18, Simrad) was faulty. The following describes the measurements conducted.  

(ref:impedance-plots) The magnitude of impedance ($|Z|$, $\Omega$; panel a), phase ($\theta$, $^\circ$; panel b), and conductance ($G$, $S$; panel c) versus frequency, and susceptance ($B$, $S$) versus $G$ (admittance circle; panel d), for each transducer quadrant (various colors) in water at the time of calibration.

```{r impedance-plots,fig.cap='(ref:impedance-plots)',out.height="7in",fig.pos='H'}
if (process.zmux) {
...
Impedance was measured using a Precision LCR Meter (E4980A, Agilent). The complex impedance was measured at 201 frequencies spanning a user-specified bandwidth. The measurements were automated using a computer-controlled, custom-made multiplexer that switches the individual transducer quadrants between an echosounder and the LCR meter. To rule out a possible fault caused by the multiplexer, impedance measurements were obtained with the transducer connected: (1) through the multiplexer, (2) directly through the transducer’s Amphenol connector, and (3) directly to the quadrant 1 wires on the transducer cable. In all cases, the quadrant 1 impedance was abnormal.  
...
```{r impedance-es18-air,fig.cap='(ref:impedance-es18-air)',out.height="4in",fig.pos='H'}
...
```{r impedance-es18-water-zmux,fig.cap='(ref:impedance-es18-water-zmux)',out.height="4in",fig.pos='H'}
...
```{r impedance-es18-water-amphenol,fig.cap='(ref:impedance-es18-water-amphenol)',out.height="4in",fig.pos='H'}
...
```{r impedance-es18-water-direct,fig.cap='(ref:impedance-es18-water-direct)',out.height="4in",fig.pos='H'}
...
During Leg I, _`r survey.vessel`_ coordinated sampling with F/V _Lisa Marie_, which conducted acoustic and purse seine sampling in shallow, nearshore waters (within ~ 5 nmi of the coast) between Cape Flattery, WA and the OR/CA border. Aboard _Lisa Marie_, SWFSC’s split-beam echosounder (Simrad EK60 GPT) was connected to the vessel's 38 kHz transducer (Simrad ES38-B), which was calibrated while on anchor in Grays Harbor (46.9236, -124.1181) on 8 May. Transmit pulses from the echosounder and sonar were not synchronized. From 20 June to 1 July, _Lisa Marie_ conducted echosounder-sonar transects between the eastern ends of _`r survey.vessel`_’s acoustic transects and shore. Each day, approximately **2** purse seine sets were conducted to provide information about size and species composition. All specimens were frozen and later processed by the Washington Department of Fish and Wildlife (WA-DFW). On 21 June, Greg Shaughnessy of Ocean Gold Seafoods embarked _`r survey.vessel`_ to observe survey operations. At the same time, Josiah Renfree boarded _Lisa Marie_ to remedy minor issues with her echosounder before rejoining _`r survey.vessel`_. At ~1900 on 25 June, Mr. Shaughnessy disembarked and was put ashore in Westport, WA via _`r survey.vessel`_'s skiff.  
...
During Leg IV, _`r survey.vessel`_ coordinated sampling with F/V _Long Beach Carnage_, which conducted acoustic and purse seine sampling in shallow, nearshore waters (within ~ 5 nmi of the coast) in the Southern CA Bight between Pt. Conception and San Diego and around the northern Channel Islands. Aboard _Long Beach Carnage_, the SWFSC's multi-frequency EK60 GPTs (70, 120, 200, and 333 kHz; Simrad) were configured with the SWFSC's split-beam multi-frequency transducer array (MTA4, ES38-12, ES70-7C, ES120-7C and ES200-7C; Simrad) mounted on the bottom of a pole. The transducers were calibrated on 18 May in the SWFSC's Ocean Technology Development Tank. Transmit pulses from the echosounder and sonar were not synchronized. From 26 August to **6** September, _Long Beach Carnage_ conducted echosounder-sonar transects between the eastern ends of _`r survey.vessel`_’s acoustic transects and shore. Each day, approximately **2** purse seine sets were conducted to provide information about size and species composition. **All specimens were [DESCRIBE SAMPLE PROCESSING PROCEURES, IF ANY].**  
