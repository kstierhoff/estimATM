...
The echosounder data from the USVs were erroneously timestamped (UTC+7 hrs), causing the correctly timestamped (UTC) navigation data to be mismatched. The erroneous timestamps were corrected in post-processing using the Fileset Time Offset parameter in Echoview.
...
**[Any differences between processing templates for nearshore vessels?]**
...
The core survey region spanned an area from approximately `r survey.landmark.n`, to `r survey.landmark.s` (**Fig. \@ref(fig:station-sampling)**), with `r nrow(nasc.summ)` east-west transects totaling `r prettyNum(sum(nasc.summ$distance), digits = 1, big.mark = ",")` nmi, and `r nrow(trawl.summ)` Nordic trawls. 

In the nearshore region, F/V _Lisa Marie_ surveyed from approximately Cape Flattery the WA-OR border (**Fig. \@ref(fig:nasc-seine-lm)**), with `r nrow(nasc.summ.lm.ns)` east-west transects totaling `r sprintf("%.0f",sum(nasc.summ.lm.ns$distance))` nmi and `r nrow(lm.sets)` purse seine sets. The USV (SD-1047) surveyed between Cape Flattery and Pt. Conception, with `r nrow(nasc.summ.sd.ns)` east-west transects totaling `r sprintf("%.0f", sum(nasc.summ.sd.ns$distance))` nmi (**Fig. \@ref(fig:nasc-trawl-sd)**). F/V _Long Beach Carnage_ surveyed from approximately Pt. Conception to San Diego, including the nearshore waters around Santa Cruz and Santa Catalina Islands  (**Fig. \@ref(fig:nasc-seine-lbc)**), with `r nrow(nasc.summ.lbc.ns)` east-west transects totaling `r sprintf("%.0f",sum(nasc.summ.lbc.ns$distance))` nmi and `r nrow(lbc.sets)` purse seine sets.

In the offshore region, between Florence, OR and San Diego, _`r survey.vessel`_ surveyed `r num2words(n_distinct(nasc.summ.rl.os$transect))` east-west transects totaling `r prettyNum(sum(nasc.summ.rl.os$distance), digits = 1, big.mark = ",")` nmi, and two USVs (SD-1045 and SD-1046) surveyed `r n_distinct(nasc.summ.sd.os$transect)` east-west transects totaling `r prettyNum(sum(nasc.summ.sd.os$distance), digits = 1, big.mark = ",")` nmi.  
...
During Leg I (from 20 June to 1 July), _`r survey.vessel`_ coordinated sampling with F/V _Lisa Marie_ between Cape Flattery, WA and the OR-CA border. On 21 June, Greg Shaughnessy (Ocean Gold Seafoods) embarked _`r survey.vessel`_ to observe survey operations. At the same time, Josiah Renfree boarded _Lisa Marie_ to remedy minor issues with the echosounder before rejoining _`r survey.vessel`_. At ~1900 on 25 June, Mr. Shaughnessy disembarked and was put ashore in Westport, WA via _`r survey.vessel`_'s skiff.  
...
During Legs **II and III (from 20 June to 24 August)**, one USV (SD-1047) conducted daytime acoustic sampling between Cape Flattery and Pt. Conception. Between approximately Tillamook, OR and Pt. Arena, transects were sampled at 10-nmi spacing to allow the USV to catch-up to _`r survey.vessel`_. During Leg IV (from 26 August to 6 September), _`r survey.vessel`_ coordinated sampling with F/V _Long Beach Carnage_ in the Southern CA Bight between Pt. Conception and San Diego and around the northern Channel Islands.  

#### Offshore sampling

**[Add details here. See biomass report for sample text.]**
...
+ Researchers at NWFSC detected an erroneous timestamp in the echosounder data collected by Saildrone, but after initial data processing had been completed (see **Appendix \@ref(appendix-nearshore-acoustic-sampling-saildrone)**).  
