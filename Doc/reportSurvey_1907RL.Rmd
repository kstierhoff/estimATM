...
pacman::p_load(tidyverse,swfscMisc,readr,pander,kableExtra,bookdown,
          ...
          stringr,xtable,devtools,gdata,reshape2,lubridate,
          scatterpie,forcats,here,viridis,rnaturalearth,rworldmap,rworldxtra)
...
axis.limits   <- c(-8, 8) # set manual limits for plot scales; for 18 kHz, -8 to 8 encompases the entire beam pattern
...
```{r process-cal-all}
if (process.cal) {
  source(here("Code/cal_extract_all.R"))
} else {
  load(here("Data/cal/cal_results_all.Rdata"))
}

if (save.figs) {
  source(here("Code/cal_ts_plot.R"))
}
```  
...
if (length(cal.noise) == 1) { 
  cal.params$noise <- rep("N/A",length(cal.files))
}else{
  cal.params$noise <- cal.noise
}
...
save(all.output,
     file = here("Output/cal/cal_output_table.Rdata"))

write.csv(all.output,
          file = here("Output/cal/cal_output_table.csv"), quote = F, row.names = F)
```
...
tsu.scatter <- ggplot(filter(cal.pings, outlier == 0), aes(athw, along)) +
  ...
  scale_colour_viridis(name = expression(paste(italic(TS)[u]," (dB)",sep = "")),
                       option = "magma") +
  scale_x_continuous('\nAthwartship Beam Angle (deg)',limits = axis.limits,
                     breaks = seq(min(axis.limits), max(axis.limits), 2)) +
...
# Save TS_c plot 
ggsave(here("Figs/fig_TSu_scatter.png"), tsu.scatter, width = 9, height = 6)
...
# Save TS_c plot 
ggsave(here("Figs/fig_TSrel_scatter.png"),tsc.scatter,width = 9,height = 6)
```
...
# Filter to remove bad data values
  ...
  # Save nav data
  save(nav, file = here("Data/nav/vessel_nav.Rdata"))
}  else {
  # Load nav data
