...
F/V _Lisa Marie_ surveyed the nearshore area from approximately Cape Flattery the WA-OR border (**Fig. \@ref(fig:nasc-seine-lm)**), with `r nrow(nasc.summ.lm)` east-west transects totaling `r sprintf("%.0f",sum(nasc.summ.lm$distance))` nmi, and `r nrow(lm.sets)` purse seine sets.  

F/V _Long Beach Carnage_ surveyed the nearshore area from approximately Pt. Conception to San Diego, including the nearshore areas around Santa Cruz and Santa Catalina Islancs  (**Fig. \@ref(fig:nasc-seine-lbc)**), with `r nrow(nasc.summ.lbc)` east-west transects totaling `r sprintf("%.0f",sum(nasc.summ.lbc$distance))` nmi, and `r nrow(lbc.sets)` purse seine sets.

### Distribution of CPS {#appendix-nearshore-results-distribution}

F/V _Lisa Marie_ catch: (**Fig. \@ref(fig:nasc-seine-lm)**).  

F/V _Long Beach Carnage_ catch: (**Fig. \@ref(fig:nasc-seine-lbc)**).  

\newpage

(ref:nasc-seine-lm) Nearshore survey transects conducted by F/V _Lisa Marie_ overlaid with the distribution of 38-kHz integrated backscattering coefficients ($s_A$, m^2^ nmi^-2^;  averaged over 2000-m distance intervals and from `r int.start`- to `r cps.depth`-m deep) ascribed to CPS (left) and the proportions, by weight, of CPS species in each purse seine set (black points indicate sets with no CPS).
...
(ref:nasc-seine-lbc) Nearshore survey transects conducted by F/V _Long Beach Carnage_ overlaid with the distribution of 38-kHz integrated backscattering coefficients ($s_A$, m^2^ nmi^-2^;  averaged over 2000-m distance intervals and from `r int.start`- to `r cps.depth`-m deep) ascribed to CPS (left) and the proportions, by weight, of CPS species in each purse seine set (black points indicate sets with no CPS).
