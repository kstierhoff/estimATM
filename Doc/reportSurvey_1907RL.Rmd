...
cufes.plot.ofe <- st_as_sf(cufes, coords = c("StartLongitude","StartLatitude"), crs = crs.geog) %>% 
  st_transform(crs = crs.proj) %>% 
...
cufes.plot.ofe <- project_sf(cufes.plot.ofe, crs.proj)
...
nasc.plot.all   <- bind_rows(nasc.cps.sf, nasc.krill.sf) %>% 
  filter(NASC >= 200)
```
...
nasc.tx.sf <- st_as_sf(nasc.cps, coords = c("long","lat"), crs = crs.geog) %>% 
  select(transect) %>% 
  group_by(transect) %>% 
  summarise(do_union = F) %>% 
  st_cast("LINESTRING") %>% 
...
if (save.figs) {
  ...
  survey.plan <- base.map +    
    ...
    geom_sf(data = transects.sf, aes(colour = Type), show.legend = "line") +
