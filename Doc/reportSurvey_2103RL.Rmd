...
Juan P. Zwolinski^1,2^, Noelle M. Bowlin^1^, Brad E. Erisman^1^, Gabriel E. Johnson^1,3^, Scott A. Mau^1^, David W. Murfin^1^, Josiah S. Renfree^1^, Brittany D. Schwartzkopf^1^, Thomas S. Sessions^1^, Kevin L. Stierhoff^1^, Lanora Vasquez^1^, William Watson^1^, and David A. Demer^1^
...
The survey data were collected by members of the Fisheries Resources Division at the SWFSC, and scientists and staff of the California Wetfish Producers Association (CWPA).  
...
* D. Porzio (CDFW, shore-side processing)
...
(ref:sardine-habitat) Distributions of a) potential habitat for the northern stock of Pacific Sardine [@Zwolinski2011], which is a function of b) chlorophyll-a concentration and c) sea surface temperature (SST). The images are composites of data from 8 days, 23 March to 1 April, 2021. The potential habitat categories (panel a), based on data from spring CalCOFI surveys between 1998 and 2009, reflect the probabilities of finding at least one sardine egg in a standard CUFES samples. The "good" and "optimal" areas collectively include 90% of the sardine biomass, and the "bad" and "unsuitable" areas contain the remaining 10%.
...
On _Long Beach Carnage_, the SWFSC's multi-frequency General Purpose Transceivers (38-, 70-, 120-, and 200-kHz Simrad EK60 GPTs; Kongsberg) were configured with the SWFSC's split-beam transducers (Simard ES38-12, ES70-7C, ES120-7C and ES200-7C; Kongsberg) mounted in a multi-frequency transducer array (MTA4) on the bottom of a pole (**Fig. \@ref(fig:cb-config-lbc)**).  
...
The echosounders were calibrated in a tank at the SWFSC. The beam model results were entered into the GPT-control software and are presented in **Table \@ref(tab:cal-results-lbc)**. The calibration parameters were adjusted for local environmental conditions using UCTD data collected from _`r survey.vessel`_.  

(ref:cal-results-lbc) General Purpose Transceiver (Simrad EK60 GPT; Kongsberg) calibrated beam model results. Prior to the survey, calibrated on-axis gain ($G_0$), beam angles and angle offsets, and $S_a$ Correction ($S_\mathrm{a}\mathrm{corr}$) values were entered into the GPT-control software (Simrad EK80; Kongsberg). **[Remove empty rows; adjust sig figs]**
...
Northern Anchovy catches were predominant, by weight, in trawl samples collected throughout the survey area (**Fig. \@ref(fig:nasc-cufes-trawl)c**). Jack Mackerel were present in several trawls offshore in the SCB, and one near Long Beach (**Fig. \@ref(fig:nasc-cufes-trawl)c**). Pacific Sardine were caught in nine trawls; however, catches were small and not visible except in one trawl haul offshore of the northern Channel Islands (**Fig. \@ref(fig:nasc-cufes-trawl)c**). Pacific Mackerel were caught in two hauls in the SCB, but are not visible at this scale (**Fig. \@ref(fig:nasc-cufes-trawl)c**). Overall, the `r nrow(haul)` trawls captured a combined `r prettyNum(sum(cluster.summ.wt$AllCPS),big.mark=",",digits=3)` kg of CPS (`r prettyNum(sum(cluster.summ.wt$'Anchovy'),big.mark=",",digits=3)` kg of Northern Anchovy, `r prettyNum(sum(cluster.summ.wt$'Sardine'),big.mark=",",digits=3)` kg of Pacific Sardine, `r prettyNum(sum(cluster.summ.wt$'PacMack'),big.mark=",",digits=3)` kg of Pacific Mackerel, and `r prettyNum(sum(cluster.summ.wt$'JackMack'),big.mark=",",digits=3)` kg of Jack Mackerel; no Pacific Herring were collected; **Appendix \@ref(appendix-trawl-sampling)**).  
...
The principal objectives of the `r survey.das`-day, Spring 2021 CCE Survey were to were to estimate the biomasses and distributions of the central stock of Northern Anchovy and, as possible, produce estimates for other CPS in the survey area. With the benefit of favorable weather and few technical problems, _`r survey.vessel`_ surveyed the entire planned area from San Francisco to San Diego and _Long Beach Carnage_ surveyed the nearshore region from Pt. Conception to San Diego, and around Santa Catalina and Santa Cruz Islands.  
