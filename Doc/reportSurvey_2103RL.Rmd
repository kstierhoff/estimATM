...
**Report on the `r survey.name.long` (`r survey.name`), `r survey.start` to `r survey.end` `r survey.year`, conducted aboard NOAA ship _`r survey.vessel.long`_ and the fishing vessel _Long Beach Carnage_**  

Juan P. Zwolinski^1,2^, Noelle M. Bowlin^1^, Brad E. Erisman^1^, Gabriel E. Johnson^1,3^, Scott A. Mau^1^, David W. Murfin^1^, Josiah S. Renfree^1^, Brittany D. Schwartzkopf^1^, Thomas S. Sessions^1^, Kevin L. Stierhoff^1^, Lanora Vasquez^1^, William Watson^1^, and David A. Demer^1^ **[add authors as appropriate]**
...
The survey data were collected by members of the Fisheries Resources Division at the SWFSC, and scientists and staff of the California Wetfish Producers Association (4-CWPA).  
...
* Leg I:   N. Bowlin, M. Craig, B. Erisman, M. Human, B. Schwartzkopf, and A. Thompson
* Leg II:  A. Freire, L. Giuseffi, V. Hermanson, K. James, O. Snodgrass, and W. Watson

**Purse-seine Sampling:**  

* J. van Noord^4^ [**Update with CWPA staff and LBC crew**]
...
(ref:anchovy-distribution) The distributions of Northern Anchovy eggs sampled by the CUFES during spring surveys from 2017 (left) through 2019 (right). Eggs indicate the presence of spawning Northern Anchovy and their distributions were used to delineate the spring 2021 survey area.
...
(ref:sardine-habitat) Distribution of potential habitat for the northern stock of Pacific Sardine at the  beginning (a), middle (b, c), and end (d) of the survey. Areas in white correspond to no available data, e.g., cloud coverage preventing satellite-sensed observations.
...
(ref:survey-plan) Planned core-area transect lines (solid black lines) sampled by _`r survey.vessel`_ and nearshore transect lines (solid magenta lines) sampled by F/V _Long Beach Carnage_ . Isobaths (light gray lines) are placed at 50, 200, 500, and 2,000 m (or approximately 25, 100, 250, and 1,000 fathoms).
...
On _Long Beach Carnage_, the SWFSC's multi-frequency echosounders (38-, 70-, 120-, and 200-kHz EK60 GPTs; Simrad) were configured with the SWFSC's split-beam transducers (ES38-12, ES70-7C, ES120-7C and ES200-7C; Simrad) mounted in a multi-frequency transducer array (MTA4) on the bottom of a pole (**Fig. \@ref(fig:cb-config-lbc)**).  

(ref:cb-config-lbc) The four-frequency scientific echosounder system aboard F/V _Long Beach Carnage_ included four Simrad EK60 GPTs (left) and four Simrad split-beam transducers with center frequencies of 200, 120, 38 and 70 kHz (top to bottom; right).
...
Each day, after the transects had been surveyed, _Long Beach Carnage_ collected CPS samples by fishing up to three times in the vicinity of randomly selected CPS schools observed in the echograms. The samples were set to provide information about size, age, and species composition of fishes observed in the nearshore region. The purse seine net was is 200 m-long and 27 m-deep net with mostly 17 mm-wide mesh. A small section on the backend of the net had 25 mm-wide mesh.  

For every set, three dip net samples were collected from the purse seine net. Each dip net sample was put in a bag and tagged with a date, time of capture, latitude and longitude, transect line number, school and dip net numbers, and estimated tonnage of entire school. Ashore, CDFW biologists processed the samples according to the following protocol:  
...
+ The contents of all dip nets in one set were combined, and as many as 50 fish of each species were selected randomly.
+ From each random sample, the following statistics were measured:
  - total sample weight recorded to the nearest gram,
  -	individual fish weight recorded to the nearest gram,
  - individual fish length recorded using standard length for Pacific Sardine and Northern Anchovy, and fork length for mackerels, and
  -	maturity stage.
  
Otoliths were extracted from randomly selected specimens.  
...
## Calibration of Echosounder used aboard _Long Beach Carnage_ {#results-echosounder-calibration-lbc}  
For _Long Beach Carnage_, the echosounders were calibrated in a tank at the SWFSC. The beam model results were entered into the GPT-control software and are presented in **Table \@ref(tab:cal-results-lbc)**. The calibration parameters were adjusted for local environmental conditions using UCTD data collected from _`r survey.vessel`_.  

(ref:cal-results-lbc) General purpose transceiver (EK60 GPT, Simrad) calibrated beam model results. Prior to the survey, calibrated on-axis gain ($G_0$), beam angles and angle offsets, and $S_a$ Correction ($S_\mathrm{a}\mathrm{corr}$) values were entered into the GPT-control software (EK80, Simrad).
...
The nearshore region spanned an area from approximately Pt. Conception to San Diego, including around Santa Cruz and Santa Catalina Islands, and was surveyed by _Long Beach Carnage_ (**Fig. \@ref(fig:nasc-seine-lbc)**). A total of `r nrow(nasc.summ.lbc.ns)` east-west transects totaling `r sprintf("%.0f",sum(nasc.summ.lbc.ns$distance))` nmi and `r nrow(lbc.sets)` purse seine sets were completed.  
...
Northern Anchovy catches were predominant, by weight, in trawl samples collected throughout the survey area (**Fig. \@ref(fig:nasc-cufes-trawl)c**). Jack Mackerel were present in several trawls offshore in the SCB, and one near Long Beach (**Fig. \@ref(fig:nasc-cufes-trawl)c**). Pacific Sardine were caught in nine trawls; however, catches were small and not visible except in one trawl haul offshore of the northern Channel Islands (**Fig. \@ref(fig:nasc-cufes-trawl)c**). Pacific Mackerel were caught in two hauls in the SCB but are not visible at this scale (**Fig. \@ref(fig:nasc-cufes-trawl)c**). Overall, the `r nrow(trawl.summ)` trawls captured a combined `r prettyNum(haul.CPS.kg, digits = 1, big.mark = ",")` kg of CPS (`r prettyNum(haul.Sardine.kg, digits = 1, big.mark = ",")` kg Pacific Sardine, `r prettyNum(haul.Anchovy.kg, digits = 1, big.mark = ",")` kg Northern Anchovy, `r prettyNum(haul.JackMack.kg, digits = 1, big.mark = ",")` kg Jack Mackerel, and `r prettyNum(haul.PacMack.kg, digits = 1, big.mark = ",")` kg Pacific Mackerel; **Appendix \@ref(appendix-trawl-sampling)**).  
...
Northern Anchovy and Pacific Sardine were the predominant CPS collected in purse seines set by _Long Beach Carnage_ (**Fig. \@ref(fig:nasc-seine-lbc)b**). A few purse seine samples contained Pacific Mackerel south of Long Beach (**Fig. \@ref(fig:nasc-seine-lbc)b**).
...
(ref:nasc-seine-lbc) Nearshore survey transects conducted by _Long Beach Carnage_ overlaid: with the distribution of 38-kHz integrated backscattering coefficients ($s_A$, m^2^ nmi^-2^; averaged over 2000-m distance intervals and from `r int.start`- to `r cps.depth`-m deep) ascribed to CPS (a) and the proportions, by weight, of CPS in each purse seine set or trawl cluster later used to apportion acoustic backscatter (b). Species with low catch weights are not visible at this scale.
...
The principal objectives of the `r survey.das`-day, Spring 2021 CCE Survey were to survey the central stock of Northern Anchovy and, as possible, survey Pacific Sardine, Pacific Mackerel, and Jack Mackerel. With the benefit of favorable weather and few technical problems, _`r survey.vessel`_ surveyed the entire planned area from San Francisco to San Diego and _Long Beach Carnage_ surveyed the nearshore region from Pt. Conception to San Diego, and around Santa Catalina and Santa Cruz Islands.  

Northern Anchovy from the central stock dominated the egg samples and trawl catches in the core area. In addition to Northern Anchovy, the nearshore purse-seine catches in the Southern California Bight and off Santa Cruz Island included Pacific Sardine. Throughout the survey, catches of Jack and Pacific Mackerel were relatively scarce.  
...
We thank the crew members of NOAA Ship _`r survey.vessel`_, as well as the scientists that participated in the sampling operations at sea. We thank Captains Rich Ashley and Tom Brinton, John Marcopolous, and William Hargrave (_Long Beach Carnage_) for their coordination and cooperation during the nearshore sampling. We thank Diane Pleschner-Steele for contracting the _Long Beach Carnage_ to conduct the nearshore survey. We thank Joel van Noord for conducting the biological sampling at sea aboard _Long Beach Carnage_. We also thank Kelly Kloos, Trevor Stocking, Diego Aceituno, Dane McDermott, Kyle Mooers, Jax Mikkelsen, Leeanne Laughlin, Chelsea Protasio, and Michelle Horeczko (from CDFW) for their efforts to coordinate with and process samples from F/V _Long Beach Carnage_. Critical reviews by **[List of Reviewers]** improved this report.  
