...
CUFES samples were dominated by Jack Mackerel eggs north of Monterey Bay, CA, and to the south by Northern Anchovy (**Fig. \@ref(fig:nasc-cufes-trawl)b**). Pacific Sardine eggs were more scarce, observed offshore between San Francisco, CA to Monterey Bay, CA, and throughout the SCB. The SCB contained a mixture of eggs from the three species, with Northern Anchovy found predominantly nearshore with Pacific Sardine and Jack Mackerel more offshore, apart from Pacific Sardine eggs in samples nearshore off San Diego, CA. Note that due to the cancellation of Leg I on _`r survey.vessel`_, no CUFES samples exist north of Cape Mendocino, CA. 

Surface trawls, conducted by _`r survey.vessel`_ south of Cape Mendocinco, CA, contained Pacific Sardine, generally mixed with Jack Mackerel, between Cape Mendocino, CA to Bodega Bay, CA and in the SCB (**Fig. \@ref(fig:nasc-cufes-trawl)c**). Northern Anchovy were collected throughout the region south of Bodega Bay, CA. Jack Mackerel were most abundant between Cape Mendocino, CA to Bodega Bay, CA; and south of Morro Bay, CA, predominantly in the SCB. Pacific Mackerel were only caught in nearshore trawls off San Clemente, CA and offshore of Punta Colonet, Baja California. The combined catches of the `r nrow(trawl.summ)` trawls included `r prettyNum(haul.CPS.kg, digits = 1, big.mark = ",")` kg of CPS (`r prettyNum(haul.Sardine.kg, digits = 1, big.mark = ",")` kg Pacific Sardine, `r prettyNum(haul.Anchovy.kg, digits = 1, big.mark = ",")` kg Northern Anchovy, `r prettyNum(haul.JackMack.kg, digits = 1, big.mark = ",")` kg Jack Mackerel, `r prettyNum(haul.PacMack.kg, digits = 1, big.mark = ",")` kg Pacific Mackerel, `r prettyNum(haul.PacHerring.kg, digits = 1, big.mark = ",")` kg Pacific Herring, and `r prettyNum(haul.RndHerring.kg, digits = 1, big.mark = ",")` kg Round Herring; **Appendix \@ref(appendix-trawl-sampling)**).  

Purse seines, conducted by _Lisa Marie_ north of Bodega Bay, CA, caught Pacific Sardine between the WA/OR border to Fort Bragg, CA.; Northern Anchovy off of Bodega Bay, CA and near Cape Blanco, OR; Jack Mackerel between northern Oregon to Bodega Bay, CA; and Pacific Herring predominantly off Washington (**Fig. \@ref(fig:nasc-cufes-trawl)d**). The combined catches of the `r nrow(filter(haul.pie, vessel.name == "LM"))` purse seine sets included `r prettyNum(sum(filter(set.summ.wt, vessel.name == "LM")$AllCPS, na.rm = T), digits = 1, big.mark = ",")` kg of CPS (`r prettyNum(sum(filter(set.summ.wt, vessel.name == "LM")$Sardine, na.rm = T), digits = 1, big.mark = ",")` kg Pacific Sardine, `r prettyNum(sum(filter(set.summ.wt, vessel.name == "LM")$Anchovy, na.rm = T), digits = 1, big.mark = ",")` kg Northern Anchovy, `r prettyNum(sum(filter(set.summ.wt, vessel.name == "LM")$JackMack, na.rm = T), digits = 1, big.mark = ",")` kg Jack Mackerel, `r prettyNum(sum(filter(set.summ.wt, vessel.name == "LM")$PacMack, na.rm = T), digits = 1, big.mark = ",")` kg Pacific Mackerel, `r prettyNum(sum(filter(set.summ.wt, vessel.name == "LM")$PacHerring, na.rm = T), digits = 1, big.mark = ",")` kg Pacific Herring, and `r prettyNum(sum(filter(set.summ.wt, vessel.name == "LM")$RndHerring, na.rm = T), digits = 1, big.mark = ",")` kg Round Herring; **Appendix \@ref(appendix-trawl-sampling)**).

\newpage  
\blandscape  

(ref:nasc-cufes-trawl) Survey transects overlaid with the distributions of: 38-kHz integrated backscattering coefficients ($s_A$, m^2^ nmi^-2^;  averaged over 2000-m distance intervals and from `r int.start`- to `r cps.depth`-m deep) ascribed to CPS (a); egg densities (eggs m^-3^) for Northern Anchovy, Jack Mackerel, and Pacific Sardine from the CUFES (b); and proportions, by weight, of CPS species in each _`r survey.vessel`_ trawl (c) or _Lisa Marie_ seine catch (d; black points indicate trawls with no CPS). Species with low catch weights are not visible at this scale.
...
Acoustic backscatter sampled by _Long Beach Carnage_ was observed throughout the entire nearshore survey area off California between Bodega Bay and San Diego, CA, including around Santa Cruz and Santa Catalina Islands (**Fig. \@ref(fig:nasc-seine-lbc)a**).  
...
The principal objectives of the `r survey.das`-day Summer 2022 CCE Survey were to survey the northern stock of Pacific Sardine and the northern and central stocks of Northern Anchovy. Then, as possible, to survey the stocks of Pacific Mackerel, Jack Mackerel, Pacific Herring, Pacific Round Herring, and the southern stock of Pacific Sardine. The combined sampling from _`r survey.vessel`_, _Lisa Marie_, _Long Beach Carnage_, and Saildrones spanned the offshore and nearshore areas from Cape Flattery, WA to Punta Baja, MX. 

The forage fish assemblage was dominated by Northern Anchovy south of Bodega Bay, CA. To the north, trawl catches from _`r survey.vessel`_ were predominantly Jack Mackerel, while purse seine sets from _Lisa Marie_ caught predominantly Pacific Sardine between the WA/OR border to Fort Bragg, CA. Based on differences in the _`r survey.vessel`_ and _Lisa Marie_ catches in the overlapping region in northern California, in addition to comparative historical results [@Renfree2022] and anectodal observations from the _Lisa Marie_ captain, it is hypothesized that purse seines are ineffective in capturing Jack Mackerel. Therefore, species proportions derived from purse seine data in regions with high Jack Mackerel abundance should be analyzed with caution. 
