...
(ref:sardine-habitat-preSurvey) Distribution of potential habitat for the northern stock of Pacific Sardine at the beginning of the survey. Areas in white correspond to no available data, e.g., cloud coverage preventing satellite-sensed observations.

```{r sardine-habitat-preSurvey,fig.cap='(ref:sardine-habitat-preSurvey)',out.height = '6in',fig.align='center',fig.pos='H'}
include_graphics(here("Figs/fig_habitat_20220628.png"))
```  
...
The core survey region spanned an area from approximately `r survey.landmark.n` to `r survey.landmark.s` (**Fig. \@ref(fig:transects-and-habitat)**). _`r survey.vessel`_, _Lisa Marie_, and the two USVs sampled `r nrow(nasc.summ)` east-west transects totaling `r prettyNum(sum(nasc.summ$distance), digits = 1, big.mark = ",")` nmi, and conducted `r nrow(trawl.summ)` Nordic trawls [**INSERT NUMBER OF LM SETS**].
