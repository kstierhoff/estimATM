...
# If 2022 summer survey, merge Lisa Marie set data (set.pie) with Lasker haul
# data (haul.pie)
if (survey.name %in% c("2207RL")) {
  
  # Retain only Lisa Marie data
  temp <- set.pie %>%
    filter(vessel.name == "LM")
  
  # Merge with Lasker data
  haul.pie <- bind_rows(haul.pie, temp)
...
```{r plot-maps-ns, eval=F}
# Plot nearshore purse seine catch and acoustic backscatter

# First load nearshore data
nasc.nearshore <- readRDS("~/EstimATM/2207RL/Data/Backscatter/LBC/nasc_vessel_LBC_nearshore.rds")

source(here("Code/plot_purseSeine_2207RL.R"))
```
