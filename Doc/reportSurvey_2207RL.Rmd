...
nasc.plot.ns <- nasc.nearshore %>%
  ...
  summarise(
    ...
    NASC = mean(cps.nasc)) %>%
  # Create bins for defining point size in NASC plots%>%
  mutate(bin       = cut(NASC, nasc.breaks.ns, include.lowest = TRUE),
         bin.level =  as.numeric(bin)) %>%
  ungroup() %>%
  project_df(to = crs.proj)
...
nasc.labels.all <- nasc.labels.ns[nasc.levels.all]
nasc.sizes.all  <- nasc.sizes.ns[nasc.levels.all]
...
The area between Cape Mendocino to Bodega Bay, CA was sampled by both _`r survey.vessel`_ and _Lisa Marie_, but had significant discrepancies in the species proportions of their catches. _`r survey.vessel`_'s night-time trawls mainly contained Jack Mackerels with a small amount of Pacific Sardine, whereas _Lisa Marie_'s day-time purse seines were composed primarily of Pacific Sardine. The two vessels both caught predominantly Northern Anchovy off of Bodega Bay, CA.
...
Acoustic backscatter sampled by _Long Beach Carnage_ was observed throughout the nearshore survey area off California between Bodega Bay and San Diego, CA, including around Santa Cruz and Santa Catalina Islands (**Fig. \@ref(fig:nasc-seine-lbc)a**).  
