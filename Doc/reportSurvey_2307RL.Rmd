...
download.hab    <- T # download habitat map from ERDDAP (F if using temporal aggregate map)
...
if (download.hab) {
  ...
  # from the Shimada
  erddap.survey.end <- "2023-11-01" # erddap.survey.end.sh
...
# Will probably remove this chunk this year, as we don't need to obtain LM set
# data to merge with Lasker data, since LM primarily only sampled nearshore.
# Rather, the processing and plotting of LM and LBC seine data will likely all
# be done using plot_purseSeine_2307RL
...
source(here("Code/processSeine_2307RL.R"))
...
```{r plot-nearshore-figure, eval=FALSE}
# Will either remove this chunk and use plot_purseSeine_2307RL to process and
# plot the seine data from both LM and LBC, or update this chunk to also load
# the Lisa Marie data. Currently it only uses LBC data.
...
```{r calc-ns-transect-summaries eval=FALSE}
...
[**HAVE KRISTEN REVIEW THIS SECTION**]
...
[**CONFIRM THAT NO CUFES WERE COLLECTED ON SHIMADA**]
[**HAVE LANORA CONFIRM THIS SECTION**]
...
[**CHECK IF ANY BONGO TOWS WERE CONDUCTED. IF SO, HAVE LANORA CONFIRM THIS SECTION**]
...
[**UPDATE AFTER OBTAINING THE POST-SURVEY CALIBRATION**]
For _Shimada_, the EK80s were calibrated by the NWFSC on [**INSERT DATE**] after completing the Integrated Ecosystem and Pacific Hake Acoustic-Trawl Survey.
...
On _Lasker_ a total of `r nrow(filter(ctd.sta, Vessel == "RL"))` CTD casts and `r nrow(uctd.sta)` UCTD casts were conducted (**Fig. \@ref(fig:station-sampling)** and **Appendix \@ref(appendix-ctd-sampling)**), and `r prettyNum(nrow(cufes.all), digits = 1, big.mark = ",")` CUFES samples collected underway. During Leg IV on _Shimada_, a total of `r nrow(filter(ctd.sta, Vessel == "SH"))` CTD casts were obtained.

A total of `r nrow(filter(ctd.sta, Vessel == "SH"))` CTD casts were obtained from the NWFSC's 2022 Integrated Ecosystem and Pacific Hake Acoustic-Trawl Survey aboard _Shimada_, which were utilized for processing the _Lisa Marie_ and USV transects off of Oregon and Washington.

[**should we include ctd casts from the hake survey used for processing the USVs?**]
(ref:station-sampling) The locations of surface trawls (white points), CTD and UCTD casts (red circles), and bongo nets (orange triangles) relative to the planned east-west acoustic transects (solid and dashed grey lines) and cruise tracks (thick black line) of _`r survey.vessel`_ and _Shimada_. The CTD casts off of Oregon and Washington were provided by the NWFSC from their 2022 Integrated Ecosystem and Pacific Hake Acoustic-Trawl Survey aboard _Shimada_ from 22 July to 25 August, 2022.
