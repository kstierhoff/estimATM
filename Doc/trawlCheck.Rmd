...
L.max <- lengths.all %>% 
  ...
  summarise(max.TL = max(totalLength_mm, na.rm = T))
...
lengths.binned <- lengths.all %>%
  filter(cruise %in% cruise.name & ship %in% cruise.ship & 
           scientificName %in% cps.spp & binned == T) %>% 
  droplevels()
...
lengths %>% 
  group_by(scientificName) %>% 
  summarise(
    TL.min = round(min(totalLength_mm, na.rm = T)/10,0),
    TL.max = round(max(totalLength_mm, na.rm = T)/10,0),
    SL.min = round(min(standardLength_mm, na.rm = T)/10,0),
    SL.max = round(max(standardLength_mm, na.rm = T)/10,0),
    FL.min = round(min(forkLength_mm, na.rm = T)/10,0),
    FL.max = round(max(forkLength_mm, na.rm = T)/10,0),
    w.min  = round(min(weightg, na.rm = T),1),
    w.max  = round(max(weightg, na.rm = T),1)) %>% 
...
if (nrow(lengths.sub) > 0) {
  ...
  lw.plot <- ggplot() + 
    ...
    geom_point(data = filter(lengths.sub, missing.length == T), 
               aes(totalLength_mm, weightg, label = label),
               shape = 21, fill = 'red',  size = 2.5, colour = NA) +
    geom_point(data = filter(lengths.sub, missing.weight == T), 
               aes(totalLength_mm, weightg, label = label),
  ...
  ggplotly(lw.plot, dynamicTicks = T) 
} else {
  print("No speciments present.")
}
```
